...
for name, (file, digest, sites) in expected.items():
    ...
    if name == 'func_80071E70':
        # Independent native oracle: enter original 1P SetCourse, then restore
        # actual 2P globals once table selection ends. Later native type-11 actor
        # filtering must still see 2P at 800731C0. Production only adjusts s5.
        oracle = baseline.replace('func_80071E70', 'reference_func_80071E70')
        marker = '    // 0x80072B80:'
        assert oracle.count(marker) == 1
        oracle = oracle.replace(marker, '    MEM_W(0, S32(0x800DAB28)) = 2;\n' + marker)
        parts.append(oracle)
...
spawn = extract(root/'RecompiledFuncsPoint0/funcs_3.c', 'func_8006AC84')
parts.append(spawn)
manifest['func_8006AC84'] = {'current_sha256': hashlib.sha256(spawn.encode()).hexdigest()}
output.write_text('\n'.join(parts))
