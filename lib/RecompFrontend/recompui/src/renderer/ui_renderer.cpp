...
public:
    RmlRenderInterface_RT64_impl(plume::RenderInterface* interface, plume::RenderDevice* device) {
        rebind_device(interface, device);
    }

    // Rml contexts retain this compatibility adapter across an API switch.
    // Their textures/compiled geometry are invalidated before the old device
    // is destroyed; CPU-side documents and virtual image data stay alive.
    void release_device_resources() {
        list_ = nullptr;
        screen_framebuffer_.reset();
        screen_descriptor_set_.reset();
        textures_.clear();
        pipeline_ms_.reset();
        pipeline_.reset();
        layout_.reset();
        ...
        pixel_shader_.reset();
        screen_texture_ms_.reset();
        screen_texture_.reset();
        screen_vertex_buffer_.reset();
        ...
        for (auto* buffer : { &upload_buffer_, &vertex_buffer_, &index_buffer_ }) {
            if (buffer->mapped_data_) buffer->buffer_->unmap();
            *buffer = DynamicBuffer{};
        }
        stale_buffers_.clear();
        copy_buffer_size_ = screen_vertex_buffer_size_ = 0;
        texture_count_ = 2;
        multisampling_ = plume::RenderMultisampling{};
        scissor_x_ = scissor_y_ = scissor_width_ = scissor_height_ = 0;
        window_width_ = window_height_ = 0;
        scissor_enabled_ = false;
        projection_mtx_ = transform_ = mvp_ = Rml::Matrix4f::Identity();
        interface_ = nullptr;
        device_ = nullptr;
    }

    void rebind_device(plume::RenderInterface* interface, plume::RenderDevice* device) {
    ...
    bool LoadTexture(Rml::TextureHandle& texture_handle, Rml::Vector2i& texture_dimensions, const Rml::String& source) override {
        ...
        ImageFromBytes file_image{};
        ImageFromBytes* image = nullptr;
        if (it == image_from_bytes_map.end()) {
            if (!source.empty() && source.front() == '?') {
                // Virtual thumbnails may be registered later. Retain their
                // existing transparent placeholder behavior.
                texture_handle = 1;
                texture_dimensions.x = 1;
                texture_dimensions.y = 1;
                return true;
            }
            if (!asset_paths::readFileTexture(source, file_image.bytes)) {
                Rml::Log::Message(Rml::Log::LT_WARNING, "Unable to read frontend image file '%s'.", source.c_str());
                return false;
            }
            file_image.type = ImageType::File;
            file_image.name = source;
            image = &file_image;
        } else {
            image = &it->second;
        }
        ...
        if (texture == nullptr) {
            Rml::Log::Message(Rml::Log::LT_WARNING, "Unable to decode frontend image '%s'.", source.c_str());
            return false;
        }
...
void recompui::RmlRenderInterface_RT64::release_device_resources() {
    if (impl) impl->release_device_resources();
}

void recompui::RmlRenderInterface_RT64::rebind_device(plume::RenderInterface* interface, plume::RenderDevice* device) {
    impl->rebind_device(interface, device);
}
