...
namespace recompui {
    ...
    RadioOption::RadioOption(ResourceId rid, Element *parent, std::string_view name, uint32_t index) : Element(rid, parent, Events(EventType::MouseButton, EventType::Click, EventType::Focus, EventType::Hover, EventType::Enable, EventType::Update), "label", true) {
        ...
        hover_style.set_background_color(theme::color::SecondaryA5);
        checked_style.set_color(theme::color::Text);
        checked_style.set_border_color(theme::color::Secondary);
        pulsing_style.set_border_color(theme::color::Secondary);
        pulsing_style.set_background_color(theme::color::SecondaryA20);
        pulsing_style.set_color(theme::color::Text);
    ...
    void RadioOption::process_event(const Event &e) {
        ...
        case EventType::Click:
            if (is_enabled() && pressed_callback) pressed_callback(index);
            break;
    ...
    void Radio::option_selected(uint32_t index) {
        if (index >= options.size() || !options[index]->is_enabled()) return;
