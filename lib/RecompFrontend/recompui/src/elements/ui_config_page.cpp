...
namespace recompui {
    ...
    ConfigBody::ConfigBody(ResourceId rid, Element *parent) : Element(rid, parent, 0, "div", false) {
        ...
        left->set_background_color(theme::color::ModalOverlay);
        left->set_border_bottom_left_radius(theme::border::radius_lg);

        right = context.create_element<Element>(this, 0, "div", false);
        set_config_body_side_styles(right);
        right->set_background_color(theme::color::Background3);
        right->set_border_left_width(4.0f);
        right->set_border_left_color(Color{145, 220, 246, 255});
        right->set_border_bottom_left_radius(theme::border::radius_lg);
        right->set_border_bottom_right_radius(theme::border::radius_lg);
    }
