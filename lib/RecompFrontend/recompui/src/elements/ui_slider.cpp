...
namespace recompui {
        ...
        case EventType::Focus:
            {
                ...
                circle_element->set_border_color(active ? theme::color::Primary : theme::color::Secondary);
                if (active) {
                    queue_update();
                }
        ...
        case EventType::Update:
            if (is_enabled()) {
                if (circle_element->is_style_enabled(focus_state)) {
                    circle_element->set_background_color(theme::color::Secondary);
                    circle_element->set_border_color(theme::color::Primary);
                    queue_update();
                }
                else {
                    circle_element->set_background_color(theme::color::Secondary);
                    circle_element->set_border_color(theme::color::Secondary);
                }
            }
            else {
                circle_element->set_background_color(theme::color::TextInactive);
                circle_element->set_border_color(theme::color::TextInactive);
            }
            break;
    ...
    Slider::Slider(ResourceId rid, Element *parent, SliderType type) : Element(rid, parent, Events(EventType::Focus, EventType::Update, EventType::Navigate, EventType::Enable)) {
        ...
        value_label->set_color(theme::color::Warning);
...

        {
            ...
            bar_element->set_margin_top(9.0f);
            bar_element->set_border_radius(3.0f);
            bar_element->set_background_color(Color{168, 227, 228, 255});
            ...
            circle_element->set_width(24.0f);
            circle_element->set_height(24.0f);
            circle_element->set_margin_top(-9.0f);
            circle_element->set_margin_right(-12.0f);
            circle_element->set_margin_left(-12.0f);
            circle_element->set_background_color(theme::color::Secondary);
            circle_element->set_border_width(3.0f);
            circle_element->set_border_color(theme::color::Secondary);
            circle_element->set_border_radius(12.0f);
