...
#   ifndef VULKAN_OBJECT_NAMES_ENABLED
        // Allows an offline validation harness to attach a debug messenger in
        // Release builds. No messenger or validation layer is enabled here.
        VK_EXT_DEBUG_UTILS_EXTENSION_NAME,
#   endif
...
#   endif
    ...
    // This logger observes an explicitly requested loader layer. It does not
    // turn on validation, synchronization checks or GPU instrumentation itself.
    static bool environmentRequestsValidationLayer() {
...
#   else
        static const char separator = ':';
#   endif
        for (const char *begin = layers; *begin;) {
            const char *end = std::strchr(begin, separator);
            const size_t length = end ? size_t(end - begin) : std::strlen(begin);
            if (length == sizeof(name) - 1 && std::strncmp(begin, name, length) == 0) return true;
            if (!end) break;
            begin = end + 1;
        }
        return false;
    ...
    static VKAPI_ATTR VkBool32 VKAPI_CALL validationMessageToStderr(
        ...
        fprintf(stderr, "[vulkan-validation] severity=%s type=0x%X id=%s number=%d: %s\n",
            level, unsigned(type), message && message->pMessageIdName ? message->pMessageIdName : "unknown",
            message ? message->messageIdNumber : 0, message && message->pMessage ? message->pMessage : "(no message)");
        std::fflush(stderr);
        return VK_FALSE;
    }

    // Opaque checkpoint markers must outlive submitted command buffers and the
    // eventual device-loss report. Never use stack strings or renderer objects.
    static const char CheckpointBegin[] = "command-begin";
    static const char CheckpointEnd[] = "command-end";
    static const char CheckpointDispatch[] = "compute-dispatch";
    static const char CheckpointTraceRays[] = "trace-rays";
    static const char CheckpointDraw[] = "draw";
    static const char CheckpointDrawIndexed[] = "draw-indexed";
    static const char CheckpointCopyBuffer[] = "copy-buffer";
    static const char CheckpointCopyTexture[] = "copy-texture";
    static const char CheckpointResolve[] = "resolve-texture";
    static const char CheckpointBLAS[] = "build-BLAS";
    static const char CheckpointTLAS[] = "build-TLAS";
    static const char CheckpointRTWater[] = "draw-RT-water";
    static const char CheckpointRTSky[] = "draw-RT-sky";
    static const char CheckpointRTWorldShadow[] = "draw-RT-world-shadow";

    static const char *checkpointPhaseName(const void *marker) {
        const char *phases[] = { CheckpointBegin, CheckpointEnd, CheckpointDispatch,
            CheckpointTraceRays, CheckpointDraw, CheckpointDrawIndexed, CheckpointCopyBuffer,
            CheckpointCopyTexture, CheckpointResolve, CheckpointBLAS, CheckpointTLAS,
            CheckpointRTWater, CheckpointRTSky, CheckpointRTWorldShadow };
        for (const char *phase : phases) if (marker == phase) return phase;
        // A driver or another library may return a marker we do not own. Do not
        // dereference it merely because our own markers happen to be strings.
    ...
    static VkPipelineStageFlags toStageFlags(RenderBarrierStages stages, bool geometrySupported, bool rtSupported, bool asSupported) {
...
#   endif
    ...
    void *VulkanBuffer::map(uint32_t subresource, const RenderRange *readRange) {
        ...
        const uint64_t begin = readRange != nullptr ? std::min(readRange->begin, desc.size) : 0;
        const uint64_t end = readRange != nullptr ? std::min(readRange->end, desc.size) : desc.size;
        if (end > begin) {
            // VMA handles nonCoherentAtomSize and makes this a no-op on coherent
            // heaps. Map alone does not make GPU writes visible to the CPU.
            res = vmaInvalidateAllocation(device->allocator, allocation, begin, end - begin);
            if (res != VK_SUCCESS) {
    ...
    void VulkanBuffer::unmap(uint32_t subresource, const RenderRange *writtenRange) {
        const uint64_t begin = writtenRange != nullptr ? std::min(writtenRange->begin, desc.size) : 0;
        const uint64_t end = writtenRange != nullptr ? std::min(writtenRange->end, desc.size) : desc.size;
        if (end > begin) {
            const VkResult res = vmaFlushAllocation(device->allocator, allocation, begin, end - begin);
            if (res != VK_SUCCESS) fprintf(stderr, "vmaFlushAllocation failed with error code 0x%X.\n", res);
        }
        vmaUnmapMemory(device->allocator, allocation);
    }
    ...
    uint64_t VulkanAccelerationStructure::getDeviceAddress() const {
        if (vk == VK_NULL_HANDLE) return 0;
        VkAccelerationStructureDeviceAddressInfoKHR info = {};
        info.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_DEVICE_ADDRESS_INFO_KHR;
        info.accelerationStructure = vk;
        return vkGetAccelerationStructureDeviceAddressKHR(device->vk, &info);
    }
    ...
    void VulkanGraphicsPipeline::setName(const std::string &name) {
        // Keep checkpoint identity even when Vulkan object names are compiled
        // out. The marker never points into this pipeline or the caller's name.
        diagnosticCheckpointPhase = name == "WR64 RT water" ? CheckpointRTWater :
            name == "WR64 RT sky" ? CheckpointRTSky :
            name == "WR64 RT world shadow" ? CheckpointRTWorldShadow : nullptr;
        setObjectName(device->vk, VK_OBJECT_TYPE_PIPELINE, uint64_t(vk), name);
    }
...
#   endif
    ...
    void VulkanCommandList::begin() {
        ...
        diagnosticCheckpointPhase = nullptr;
        activeGraphicsCheckpointPhase = nullptr;
        diagnosticCheckpoint(CheckpointBegin);
    }

    void VulkanCommandList::end() {
        endActiveRenderPass();
        diagnosticCheckpoint(CheckpointEnd);
    ...
    void VulkanCommandList::diagnosticCheckpoint(const char *phase) {
        // Opt-in diagnostics only. Consecutive native draws share one phase,
        // avoiding a checkpoint for every triangle batch in ordinary scenes.
        const auto setCheckpoint = queue->device->cmdSetCheckpoint;
        if (setCheckpoint && phase != diagnosticCheckpointPhase) {
            setCheckpoint(vk, phase);
            diagnosticCheckpointPhase = phase;
        }
    }
    ...
    void VulkanCommandList::barriers(RenderBarrierStages stages, const RenderBufferBarrier *bufferBarriers, uint32_t bufferBarriersCount, const RenderTextureBarrier *textureBarriers, uint32_t textureBarriersCount) {
        ...
        const bool asEnabled = rtEnabled || queue->device->capabilities.rayQueries;
        VkPipelineStageFlags srcStageMask = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
        VkPipelineStageFlags dstStageMask = VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT | toStageFlags(stages, geometryEnabled, rtEnabled, asEnabled);
        ...
        for (uint32_t i = 0; i < bufferBarriersCount; i++) {
            ...
            srcStageMask |= toStageFlags(interfaceBuffer->barrierStages, geometryEnabled, rtEnabled, asEnabled);
        ...
        for (uint32_t i = 0; i < textureBarriersCount; i++) {
            ...
            srcStageMask |= toStageFlags(interfaceTexture->barrierStages, geometryEnabled, rtEnabled, asEnabled);
    ...
    void VulkanCommandList::dispatch(uint32_t threadGroupCountX, uint32_t threadGroupCountY, uint32_t threadGroupCountZ) {
        ...
        diagnosticCheckpoint(CheckpointDispatch);
    ...
    void VulkanCommandList::traceRays(uint32_t width, uint32_t height, uint32_t depth, RenderBufferReference shaderBindingTable, const RenderShaderBindingGroupsInfo &shaderBindingGroupsInfo) {
        ...
        diagnosticCheckpoint(CheckpointTraceRays);
    ...
    void VulkanCommandList::drawInstanced(uint32_t vertexCountPerInstance, uint32_t instanceCount, uint32_t startVertexLocation, uint32_t startInstanceLocation) {
        ...
        diagnosticCheckpoint(activeGraphicsCheckpointPhase ? activeGraphicsCheckpointPhase : CheckpointDraw);
    ...
    void VulkanCommandList::drawIndexedInstanced(uint32_t indexCountPerInstance, uint32_t instanceCount, uint32_t startIndexLocation, int32_t baseVertexLocation, uint32_t startInstanceLocation) {
        ...
        diagnosticCheckpoint(activeGraphicsCheckpointPhase ? activeGraphicsCheckpointPhase : CheckpointDrawIndexed);
    ...
    void VulkanCommandList::setPipeline(const RenderPipeline *pipeline) {
        ...
        case VulkanPipeline::Type::Graphics: {
            ...
            activeGraphicsCheckpointPhase = graphicsPipeline->diagnosticCheckpointPhase;
    ...
    void VulkanCommandList::copyBufferRegion(RenderBufferReference dstBuffer, RenderBufferReference srcBuffer, uint64_t size) {
        endActiveRenderPass();
        diagnosticCheckpoint(CheckpointCopyBuffer);
    ...
    void VulkanCommandList::copyTextureRegion(const RenderTextureCopyLocation &dstLocation, const RenderTextureCopyLocation &srcLocation, uint32_t dstX, uint32_t dstY, uint32_t dstZ, const RenderBox *srcBox) {
        endActiveRenderPass();
        diagnosticCheckpoint(CheckpointCopyTexture);
    ...
    void VulkanCommandList::resolveTextureRegion(const RenderTexture *dstTexture, uint32_t dstX, uint32_t dstY, const RenderTexture *srcTexture, const RenderRect *srcRect, RenderResolveMode resolveMode) {
        ...
        diagnosticCheckpoint(CheckpointResolve);
    ...
    void VulkanCommandList::buildBottomLevelAS(const RenderAccelerationStructure *dstAccelerationStructure, RenderBufferReference scratchBuffer, const RenderBottomLevelASBuildInfo &buildInfo) {
        ...
        endActiveRenderPass();
        diagnosticCheckpoint(CheckpointBLAS);
        ...
        const uint64_t scratchAlignment = queue->device->accelerationStructureProperties.minAccelerationStructureScratchOffsetAlignment;
        if (scratchAlignment == 0 || scratchBuffer.offset > interfaceScratchBuffer->desc.size ||
            buildInfo.scratchSize > interfaceScratchBuffer->desc.size - scratchBuffer.offset) {
            throw std::runtime_error("Invalid Vulkan BLAS scratch allocation.");
        }
        ...
        buildGeometryInfo.scratchData.deviceAddress = roundUp(
            vkGetBufferDeviceAddress(queue->device->vk, &scratchAddressInfo) + scratchBuffer.offset, scratchAlignment);
        ...
        // Vulkan requires one range for every geometry, not a single range
        // holding the sum of all mesh primitives.
        const size_t geometryBytes = sizeof(VkAccelerationStructureGeometryKHR) * buildInfo.meshCount;
        const size_t rangeBytes = sizeof(VkAccelerationStructureBuildRangeInfoKHR) * buildInfo.meshCount;
        if (buildInfo.meshCount == 0 || buildInfo.buildData.size() != geometryBytes + rangeBytes)
            throw std::runtime_error("Invalid Vulkan BLAS geometry ranges.");
        const auto *buildRangeInfoPtr = reinterpret_cast<const VkAccelerationStructureBuildRangeInfoKHR *>(
            buildInfo.buildData.data() + geometryBytes);
    ...
    void VulkanCommandList::buildTopLevelAS(const RenderAccelerationStructure *dstAccelerationStructure, RenderBufferReference scratchBuffer, RenderBufferReference instancesBuffer, const RenderTopLevelASBuildInfo &buildInfo) {
        ...
        endActiveRenderPass();
        diagnosticCheckpoint(CheckpointTLAS);
        ...
        if (scratchAlignment == 0 || scratchBuffer.offset > interfaceScratchBuffer->desc.size ||
            buildInfo.scratchSize > interfaceScratchBuffer->desc.size - scratchBuffer.offset) {
            throw std::runtime_error("Invalid Vulkan TLAS scratch allocation.");
        }
    ...
    void VulkanCommandQueue::executeCommandLists(const RenderCommandList **commandLists, uint32_t commandListCount, RenderCommandSemaphore **waitSemaphores, uint32_t waitSemaphoreCount, RenderCommandSemaphore **signalSemaphores, uint32_t signalSemaphoreCount, RenderCommandFence *signalFence) {
        ...
        for (uint32_t i = 0; i < waitSemaphoreCount; i++) {
            ...
            // This API carries no consumer-stage information. Upload waits
            // can precede compute, vertex fetch, fragment reads or AS builds,
            // so a color-output-only wait does not protect their consumers.
            waitStageVector.emplace_back(VK_PIPELINE_STAGE_ALL_COMMANDS_BIT);
        }
        ...
        if (res != VK_SUCCESS) {
            ...
            if (res == VK_ERROR_DEVICE_LOST)
                device->reportDeviceLossCheckpoints("vkQueueSubmit", familyIndex, queueIndex);
            // The caller must not wait on a submission that never succeeded.
            throw std::runtime_error("vkQueueSubmit failed with VkResult " + std::to_string(int(res)) + ".");
        }
    }
    ...
    void VulkanCommandQueue::waitForCommandFence(RenderCommandFence *fence) {
        ...
        if (res != VK_SUCCESS) {
            ...
            if (res == VK_ERROR_DEVICE_LOST)
                device->reportDeviceLossCheckpoints("vkWaitForFences", familyIndex, queueIndex);
            // Completion was not established. Stop before the owner resets or
            // reuses command buffers and resources still associated with it.
            throw std::runtime_error("vkWaitForFences failed with VkResult " + std::to_string(int(res)) + ".");
        }
        ...
        if (res != VK_SUCCESS) {
            ...
            if (res == VK_ERROR_DEVICE_LOST)
                device->reportDeviceLossCheckpoints("vkResetFences", familyIndex, queueIndex);
            throw std::runtime_error("vkResetFences failed with VkResult " + std::to_string(int(res)) + ".");
        }
    }
    ...
    void VulkanDevice::reportDeviceLossCheckpoints(const char *operation, uint32_t failingFamily, uint32_t failingQueue) {
        // VK_NV_device_diagnostic_checkpoints permits this query only after
        // VK_ERROR_DEVICE_LOST. Never poll it during ordinary rendering.
        if (checkpointFailureReported.exchange(true, std::memory_order_relaxed)) return;
        fprintf(stderr, "[vulkan-checkpoints] device-lost operation=%s family=%u queue=%u enabled=%u fault_info=%u\n",
            operation, failingFamily, failingQueue, unsigned(getQueueCheckpointData != nullptr),
            unsigned(getDeviceFaultInfo != nullptr));
        if (getDeviceFaultInfo) {
            // Fixed capacities avoid allocating memory in the loss path. The
            // API reports VK_INCOMPLETE if additional records were available.
            VkDeviceFaultAddressInfoEXT addresses[32] = {};
            VkDeviceFaultVendorInfoEXT vendors[16] = {};
            VkDeviceFaultCountsEXT counts = {};
            counts.sType = VK_STRUCTURE_TYPE_DEVICE_FAULT_COUNTS_EXT;
            counts.addressInfoCount = uint32_t(sizeof(addresses) / sizeof(addresses[0]));
            counts.vendorInfoCount = uint32_t(sizeof(vendors) / sizeof(vendors[0]));
            VkDeviceFaultInfoEXT info = {};
            info.sType = VK_STRUCTURE_TYPE_DEVICE_FAULT_INFO_EXT;
            info.pAddressInfos = addresses;
            info.pVendorInfos = vendors;
            // Vendor binary collection is disabled at device creation.
            const VkResult status = getDeviceFaultInfo(vk, &counts, &info);
            if (status == VK_SUCCESS || status == VK_INCOMPLETE) {
                const uint32_t addressCount = std::min(counts.addressInfoCount, uint32_t(sizeof(addresses) / sizeof(addresses[0])));
                const uint32_t vendorCount = std::min(counts.vendorInfoCount, uint32_t(sizeof(vendors) / sizeof(vendors[0])));
                fprintf(stderr, "[vulkan-fault] result=%d partial=%u addresses=%u vendors=%u description=%.*s\n",
                    int(status), unsigned(status == VK_INCOMPLETE), addressCount, vendorCount,
                    int(VK_MAX_DESCRIPTION_SIZE), info.description);
                for (uint32_t i = 0; i < addressCount; ++i) {
                    // GPU virtual addresses are diagnostic numbers, never host
                    // pointers to dereference or pass to another API.
                    fprintf(stderr, "[vulkan-fault] address_type=%s(%u) reported=0x%016llX precision=0x%016llX\n",
                        deviceFaultAddressTypeName(addresses[i].addressType), unsigned(addresses[i].addressType),
                ...
                for (uint32_t i = 0; i < vendorCount; ++i) {
                    fprintf(stderr, "[vulkan-fault] vendor_code=0x%016llX vendor_data=0x%016llX description=%.*s\n",
                        static_cast<unsigned long long>(vendors[i].vendorFaultCode),
                        static_cast<unsigned long long>(vendors[i].vendorFaultData),
                        int(VK_MAX_DESCRIPTION_SIZE), vendors[i].description);
                }
            }
            else fprintf(stderr, "[vulkan-fault] query failed result=%d\n", int(status));
        }
        if (!getQueueCheckpointData) {
            std::fflush(stderr);
            return;
        }
        // Inspect every native queue: the host that observes the failure need
        // not own the queue whose command triggered it. No GPU wait is added.
        for (uint32_t family = 0; family < queueFamilies.size(); ++family) {
            for (uint32_t index = 0; index < queueFamilies[family].queues.size(); ++index) {
                const auto &nativeQueue = queueFamilies[family].queues[index];
                if (nativeQueue.vk == VK_NULL_HANDLE) continue;
                VkCheckpointDataNV data[64] = {};
                for (auto &entry : data) entry.sType = VK_STRUCTURE_TYPE_CHECKPOINT_DATA_NV;
                uint32_t count = uint32_t(sizeof(data) / sizeof(data[0]));
                getQueueCheckpointData(nativeQueue.vk, &count, data);
                count = std::min(count, uint32_t(sizeof(data) / sizeof(data[0])));
                // Some drivers leave the supplied count unchanged on queues
                // without checkpoint data. Suppress untouched zero records,
                // retaining every nonempty marker (including unknown ones).
                uint32_t emptyCount = 0;
                for (uint32_t i = 0; i < count; ++i)
                    emptyCount += data[i].stage == 0 && data[i].pCheckpointMarker == nullptr;
                fprintf(stderr, "[vulkan-checkpoints] family=%u queue=%u records=%u empty_records=%u\n",
                    family, index, count - emptyCount, emptyCount);
                for (uint32_t i = 0; i < count; ++i) {
                    if (data[i].stage == 0 && data[i].pCheckpointMarker == nullptr) continue;
                    fprintf(stderr, "[vulkan-checkpoints] family=%u queue=%u stage=0x%08X phase=%s marker=%p\n",
                        family, index, unsigned(data[i].stage), checkpointPhaseName(data[i].pCheckpointMarker),
...
#       endif
        ...
        // A diagnostic launch may bypass the driver's persistent/internal
        // pipeline cache for this device. Unlike deleting a shared cache or
        // changing compiler flags, this leaves shader code and other apps alone.
        const char *internalCacheOption = std::getenv("PLUME_VULKAN_DISABLE_INTERNAL_CACHE");
        const bool disableInternalCacheRequested = internalCacheOption &&
            internalCacheOption[0] == '1' && internalCacheOption[1] == '\0';
        ...
        if (disableInternalCacheRequested) {
            const auto extensionAvailable = [&](const char *name) {
            ...
            const uint32_t apiVersion = std::min(physicalDeviceProperties.apiVersion, renderInterface->appInfo.apiVersion);
            const bool maintenance5Required = apiVersion < VK_API_VERSION_1_4;
            const bool dynamicRenderingRequired = apiVersion < VK_API_VERSION_1_3;
            const bool dependenciesAvailable = apiVersion >= VK_API_VERSION_1_2 &&
                (!maintenance5Required || extensionAvailable(VK_KHR_MAINTENANCE_5_EXTENSION_NAME)) &&
                (!dynamicRenderingRequired || extensionAvailable(VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME));
            if (!extensionAvailable(VK_KHR_PIPELINE_BINARY_EXTENSION_NAME) || !dependenciesAvailable) {
                fprintf(stderr, "[vulkan-internal-cache] requested_disabled=1 disabled=0 reason=required-extension-unavailable; cache test cannot continue.\n");
                std::fflush(stderr);
                throw std::runtime_error("Vulkan cache test requires VK_KHR_pipeline_binary and its extension dependencies.");
            }
            VkPhysicalDevicePipelineBinaryPropertiesKHR binaryProperties = {};
            binaryProperties.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PIPELINE_BINARY_PROPERTIES_KHR;
            VkPhysicalDeviceProperties2 properties = {};
            properties.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PROPERTIES_2;
            properties.pNext = &binaryProperties;
            vkGetPhysicalDeviceProperties2(physicalDevice, &properties);
            pipelineBinaryFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PIPELINE_BINARY_FEATURES_KHR;
            VkPhysicalDeviceFeatures2 features = {};
            features.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2;
            features.pNext = &pipelineBinaryFeatures;
            vkGetPhysicalDeviceFeatures2(physicalDevice, &features);
            if (!pipelineBinaryFeatures.pipelineBinaries || !binaryProperties.pipelineBinaryInternalCacheControl) {
                fprintf(stderr, "[vulkan-internal-cache] requested_disabled=1 disabled=0 reason=internal-cache-control-unsupported; cache test cannot continue.\n");
                std::fflush(stderr);
                throw std::runtime_error("Vulkan driver does not support disabling its internal pipeline cache.");
            }
            supportedOptionalExtensions.insert(VK_KHR_PIPELINE_BINARY_EXTENSION_NAME);
            if (maintenance5Required) supportedOptionalExtensions.insert(VK_KHR_MAINTENANCE_5_EXTENSION_NAME);
            if (dynamicRenderingRequired) supportedOptionalExtensions.insert(VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME);
            internalCacheControl.sType = VK_STRUCTURE_TYPE_DEVICE_PIPELINE_BINARY_INTERNAL_CACHE_CONTROL_KHR;
            internalCacheControl.disableInternalCache = VK_TRUE;
        }
        ...
        const bool bufferDeviceAddressFound = physicalDeviceProperties.apiVersion >= VK_API_VERSION_1_2 ||
        ...
        const bool accelerationStructureFound = bufferDeviceAddressFound &&
            supportedOptionalExtensions.find(VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME) != supportedOptionalExtensions.end() &&
            supportedOptionalExtensions.find(VK_KHR_DEFERRED_HOST_OPERATIONS_EXTENSION_NAME) != supportedOptionalExtensions.end();
        const bool rayTracingFound = accelerationStructureFound &&
            supportedOptionalExtensions.find(VK_KHR_RAY_TRACING_PIPELINE_EXTENSION_NAME) != supportedOptionalExtensions.end();
        const bool rayQueryFound = accelerationStructureFound &&
            supportedOptionalExtensions.find(VK_KHR_RAY_QUERY_EXTENSION_NAME) != supportedOptionalExtensions.end();
        if (rayTracingFound) {
        ...
        const char *checkpointOption = std::getenv("PLUME_VULKAN_CHECKPOINTS");
        const bool checkpointsRequested = checkpointOption && checkpointOption[0] == '1' && checkpointOption[1] == '\0';
        const bool checkpointsAvailable = supportedOptionalExtensions.count(VK_NV_DEVICE_DIAGNOSTIC_CHECKPOINTS_EXTENSION_NAME) != 0;
        if (!checkpointsRequested)
            supportedOptionalExtensions.erase(VK_NV_DEVICE_DIAGNOSTIC_CHECKPOINTS_EXTENSION_NAME);
        const bool deviceFaultAvailable = supportedOptionalExtensions.count(VK_EXT_DEVICE_FAULT_EXTENSION_NAME) != 0;
        VkPhysicalDeviceFaultFeaturesEXT deviceFaultFeatures = {};
        if (checkpointsRequested && deviceFaultAvailable) {
            deviceFaultFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FAULT_FEATURES_EXT;
            deviceFaultFeatures.pNext = featuresChain;
            featuresChain = &deviceFaultFeatures;
        }
        else supportedOptionalExtensions.erase(VK_EXT_DEVICE_FAULT_EXTENSION_NAME);
        ...
        if (rayQueryFound) {
            ...
            rayQueryFeatures.pNext = featuresChain;
            featuresChain = &rayQueryFeatures;
        }
        ...
        // RT64's native framebuffer shaders bind 8/16/32-bit and SDR/HDR
        // views dynamically. Reject unsupported devices before constructing
        // their formatless-write pipelines, with an actionable setup error.
        if (!deviceFeatures.features.shaderStorageImageWriteWithoutFormat) {
            fprintf(stderr, "Vulkan native framebuffer conversion requires shaderStorageImageWriteWithoutFormat.\n");
            return;
        }
        ...
        if (disableInternalCacheRequested) {
            pipelineBinaryFeatures.pNext = createDeviceChain;
            createDeviceChain = &pipelineBinaryFeatures;
            internalCacheControl.pNext = createDeviceChain;
            createDeviceChain = &internalCacheControl;
        }
        const bool deviceFaultSupported = checkpointsRequested && deviceFaultAvailable && deviceFaultFeatures.deviceFault;
        if (deviceFaultSupported) {
            deviceFaultFeatures.deviceFaultVendorBinary = VK_FALSE;
            deviceFaultFeatures.pNext = createDeviceChain;
            createDeviceChain = &deviceFaultFeatures;
        }
        else supportedOptionalExtensions.erase(VK_EXT_DEVICE_FAULT_EXTENSION_NAME);
        const bool accelerationStructureSupported = accelerationStructureFeatures.accelerationStructure &&
            bufferDeviceAddressFeatures.bufferDeviceAddress;
        const bool rayTracingSupported = rayTracingPipelineFeatures.rayTracingPipeline && accelerationStructureSupported;
        const bool rayQuerySupported = rayQueryFeatures.rayQuery && accelerationStructureSupported &&
            physicalDeviceProperties.apiVersion >= VK_API_VERSION_1_2 && renderInterface->appInfo.apiVersion >= VK_API_VERSION_1_2;
        if (rayTracingSupported) {
            rayTracingPipelineFeatures.pNext = createDeviceChain;
            createDeviceChain = &rayTracingPipelineFeatures;

        }
        if (rayTracingSupported || rayQuerySupported) {
            accelerationStructureFeatures.pNext = createDeviceChain;
            createDeviceChain = &accelerationStructureFeatures;
        }
        if (rayQuerySupported) {
            rayQueryFeatures.pNext = createDeviceChain;
            createDeviceChain = &rayQueryFeatures;
        }
        ...
        // Plume barriers do not expose queue-family ownership transfers, and
        // resources use EXCLUSIVE sharing. Keep all worker types in the same
        // universal family so copy/compute/graphics can share those resources
        // legally. Workers still use distinct native queues where available.
        pickFamilyQueue(RenderCommandListType::DIRECT, VK_QUEUE_GRAPHICS_BIT | VK_QUEUE_COMPUTE_BIT | VK_QUEUE_TRANSFER_BIT);
        queueFamilyIndices[toFamilyIndex(RenderCommandListType::COMPUTE)] = queueFamilyIndices[toFamilyIndex(RenderCommandListType::DIRECT)];
        queueFamilyIndices[toFamilyIndex(RenderCommandListType::COPY)] = queueFamilyIndices[toFamilyIndex(RenderCommandListType::DIRECT)];
        ...
        if (disableInternalCacheRequested) {
            fprintf(stderr, "[vulkan-internal-cache] requested_disabled=1 disabled=1 scope=device shader_code_unchanged=1\n");
            std::fflush(stderr);
        }

        if (checkpointsRequested && checkpointsAvailable) {
            cmdSetCheckpoint = reinterpret_cast<PFN_vkCmdSetCheckpointNV>(vkGetDeviceProcAddr(vk, "vkCmdSetCheckpointNV"));
            getQueueCheckpointData = reinterpret_cast<PFN_vkGetQueueCheckpointDataNV>(vkGetDeviceProcAddr(vk, "vkGetQueueCheckpointDataNV"));
            if (!cmdSetCheckpoint || !getQueueCheckpointData) {
                cmdSetCheckpoint = nullptr;
                getQueueCheckpointData = nullptr;
            }
        }
        if (checkpointsRequested) {
            if (deviceFaultSupported)
                getDeviceFaultInfo = reinterpret_cast<PFN_vkGetDeviceFaultInfoEXT>(vkGetDeviceProcAddr(vk, "vkGetDeviceFaultInfoEXT"));
            fprintf(stderr, "[vulkan-checkpoints] requested=1 supported=%u enabled=%u fault_supported=%u fault_enabled=%u phase_changes_only=1\n",
                unsigned(checkpointsAvailable), unsigned(cmdSetCheckpoint != nullptr),
                unsigned(deviceFaultSupported), unsigned(getDeviceFaultInfo != nullptr));
        }
        ...
        // The inline-query raster path uses SPIR-V 1.5, non-uniform texture
        // descriptors, scalar buffer layout, and an optional fragment UAV.
        capabilities.rayQueries = rayQuerySupported && descriptorIndexingSupported && scalarBlockLayoutSupported &&
            indexingFeatures.shaderSampledImageArrayNonUniformIndexing && deviceFeatures.features.fragmentStoresAndAtomics;
...
#   endif
    ...
    void VulkanDevice::setBottomLevelASBuildInfo(RenderBottomLevelASBuildInfo &buildInfo, const RenderBottomLevelASMesh *meshes, uint32_t meshCount, bool preferFastBuild, bool preferFastTrace) {
        ...
        const size_t geometryBytes = sizeof(VkAccelerationStructureGeometryKHR) * meshCount;
        buildInfo.buildData.resize(geometryBytes + sizeof(VkAccelerationStructureBuildRangeInfoKHR) * meshCount);
        ...
        auto *ranges = reinterpret_cast<VkAccelerationStructureBuildRangeInfoKHR *>(buildInfo.buildData.data() + geometryBytes);
        for (uint32_t i = 0; i < meshCount; i++) {
            ...
            ranges[i] = {};
            ranges[i].primitiveCount = geometryPrimitiveCounts[i];
        }
        ...
        const uint64_t scratchAlignment = accelerationStructureProperties.minAccelerationStructureScratchOffsetAlignment;
        if (scratchAlignment == 0) throw std::runtime_error("Vulkan AS scratch alignment is unavailable.");
        // Reserve room to align the actual device address, including callers
        // suballocating scratch at a non-aligned buffer offset.
        buildInfo.scratchSize = buildSizesInfo.buildScratchSize + scratchAlignment - 1;
    ...
    void VulkanDevice::setTopLevelASBuildInfo(RenderTopLevelASBuildInfo &buildInfo, const RenderTopLevelASInstance *instances, uint32_t instanceCount, bool preferFastBuild, bool preferFastTrace) {
        ...
        for (uint32_t i = 0; i < instanceCount; i++) {
            ...
            if (instance.bottomLevelASDeviceAddress == 0)
                throw std::runtime_error("Vulkan TLAS requires the BLAS device address, not its buffer address.");
            ...
            bufferInstance.accelerationStructureReference = instance.bottomLevelASDeviceAddress;
        }
...
#   endif
        ...
        const char *validationEnvironment = std::getenv("PLUME_VULKAN_VALIDATION");
        if (validationEnvironment && std::strcmp(validationEnvironment, "1") == 0) {
            bool layerRequested = environmentRequestsValidationLayer();
#       ifdef VULKAN_VALIDATION_LAYER_ENABLED
            layerRequested = layerRequested || createInfo.enabledLayerCount != 0;
#       endif
            bool layerAvailable = false;
            uint32_t diagnosticLayerCount = 0;
            if (layerRequested && vkEnumerateInstanceLayerProperties(&diagnosticLayerCount, nullptr) == VK_SUCCESS) {
                std::vector<VkLayerProperties> diagnosticLayers(diagnosticLayerCount);
                const VkResult layerResult = vkEnumerateInstanceLayerProperties(&diagnosticLayerCount, diagnosticLayers.data());
                if (layerResult == VK_SUCCESS || layerResult == VK_INCOMPLETE) {
                    diagnosticLayerCount = std::min(diagnosticLayerCount, uint32_t(diagnosticLayers.size()));
                    for (uint32_t i = 0; i < diagnosticLayerCount; ++i)
                        layerAvailable = layerAvailable || std::strcmp(diagnosticLayers[i].layerName, "VK_LAYER_KHRONOS_validation") == 0;
                }
            }
            const bool debugUtilsEnabled = requiredExtensions.count(VK_EXT_DEBUG_UTILS_EXTENSION_NAME) != 0 ||
                supportedOptionalExtensions.count(VK_EXT_DEBUG_UTILS_EXTENSION_NAME) != 0;
            VkResult messengerResult = VK_ERROR_EXTENSION_NOT_PRESENT;
            if (layerRequested && layerAvailable && debugUtilsEnabled) {
                ...
                if (createMessenger && destroyValidationMessenger) {
                    ...
                    messengerInfo.pfnUserCallback = validationMessageToStderr;
                    messengerResult = createMessenger(instance, &messengerInfo, nullptr, &validationMessenger);
                }
            }
            if (messengerResult != VK_SUCCESS) {
                validationMessenger = VK_NULL_HANDLE;
                destroyValidationMessenger = nullptr;
            }
            fprintf(stderr, "[vulkan-validation] requested=1 layer_requested=%u layer_available=%u debug_utils=%u messenger=%u result=%d\n",
                unsigned(layerRequested), unsigned(layerAvailable), unsigned(debugUtilsEnabled),
                unsigned(validationMessenger != VK_NULL_HANDLE), int(messengerResult));
            std::fflush(stderr);
        }
    ...
    VulkanInterface::~VulkanInterface() {
        if (instance != nullptr) {
            if (validationMessenger != VK_NULL_HANDLE && destroyValidationMessenger) {
                destroyValidationMessenger(instance, validationMessenger, nullptr);
                validationMessenger = VK_NULL_HANDLE;
            }
