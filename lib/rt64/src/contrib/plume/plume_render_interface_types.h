...
namespace plume {
    ...
    struct RenderTopLevelASInstance {
        ...
        // Vulkan AS addresses need not equal their backing buffer addresses.
        // Set from RenderAccelerationStructure::getDeviceAddress(). Zero keeps
        // the existing buffer-address contract on backends where it is valid.
        uint64_t bottomLevelASDeviceAddress = 0;
