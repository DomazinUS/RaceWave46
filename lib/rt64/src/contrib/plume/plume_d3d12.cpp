...
#include "plume_wr64_d3d12_diagnostics.h"
...
#   endif
    ...
    D3D12SwapChain::~D3D12SwapChain() {
        // Creation can fail before any back buffers have been allocated.
    ...
    bool D3D12SwapChain::present(uint32_t textureIndex, RenderCommandSemaphore **waitSemaphores, uint32_t waitSemaphoreCount) {
        // The waitable object limits queued frames, but does not replace VSync.
        // SyncInterval=0 permits flip-model frame cancellation even without
        // ALLOW_TEARING; keep each synchronized frame for a display interval.
        ...
        wr64D3D12Failure(commandQueue->device->d3d, "Present", res);
    ...
    void D3D12SwapChain::wait() {
        ...
        if (waitableObject != NULL) {
            const DWORD result = WaitForSingleObject(waitableObject, INFINITE);
            if (result == WAIT_FAILED)
                wr64D3D12Failure(commandQueue->device->d3d, "SwapChain present wait", HRESULT_FROM_WIN32(GetLastError()));
            if (wr64D3D12DiagnosticsEnabled())
                wr64D3D12Failure(commandQueue->device->d3d, "SwapChain present wait device status", commandQueue->device->d3d->GetDeviceRemovedReason());
        }
    }
    ...
    bool D3D12SwapChain::resize() {
        ...
        wr64D3D12Failure(commandQueue->device->d3d, "ResizeBuffers", res);
        if (FAILED(res)) {
...
#   endif
    ...
    void D3D12CommandQueue::executeCommandLists(const RenderCommandList **commandLists, uint32_t commandListCount, RenderCommandSemaphore **waitSemaphores, uint32_t waitSemaphoreCount, RenderCommandSemaphore **signalSemaphores, uint32_t signalSemaphoreCount, RenderCommandFence *signalFence) {
        ...
        for (uint32_t i = 0; i < waitSemaphoreCount; i++) {
            ...
            const HRESULT result = d3d->Wait(interfaceSemaphore->d3d, interfaceSemaphore->semaphoreValue);
            wr64D3D12Failure(device->d3d, "CommandQueue::Wait", result);
        }
        ...
        for (uint32_t i = 0; i < signalSemaphoreCount; i++) {
            ...
            const HRESULT result = d3d->Signal(interfaceSemaphore->d3d, interfaceSemaphore->semaphoreValue);
            wr64D3D12Failure(device->d3d, "CommandQueue::Signal semaphore", result);
        }
        
        if (signalFence != nullptr) {
            D3D12CommandFence *interfaceFence = static_cast<D3D12CommandFence *>(signalFence);
            HRESULT result = d3d->Signal(interfaceFence->d3d, interfaceFence->fenceValue);
            wr64D3D12Failure(device->d3d, "CommandQueue::Signal fence", result);
            result = interfaceFence->d3d->SetEventOnCompletion(interfaceFence->fenceValue, interfaceFence->fenceEvent);
            wr64D3D12Failure(device->d3d, "Fence::SetEventOnCompletion", result);
    ...
    void D3D12CommandQueue::waitForCommandFence(RenderCommandFence *fence) {
        ...
        const DWORD result = WaitForSingleObjectEx(interfaceFence->fenceEvent, INFINITE, FALSE);
        if (result == WAIT_FAILED)
            wr64D3D12Failure(device->d3d, "Fence wait", HRESULT_FROM_WIN32(GetLastError()));
        if (wr64D3D12DiagnosticsEnabled())
            wr64D3D12Failure(device->d3d, "Fence wait device status", device->d3d->GetDeviceRemovedReason());
    }
    ...
    D3D12Buffer::D3D12Buffer(D3D12Device *device, D3D12Pool *pool, const RenderBufferDesc &desc) {
        ...
        wr64D3D12Failure(device->d3d, "CreateResource buffer", res);
        if (FAILED(res)) {
    ...
    void *D3D12Buffer::map(uint32_t subresource, const RenderRange *readRange) {
        ...
        if (!d3d) wr64D3D12Failure(device->d3d, "Map null resource", E_POINTER);
        const HRESULT result = d3d->Map(subresource, (readRange != nullptr) ? &range : nullptr, &outputData);
        wr64D3D12Failure(device->d3d, "Map", result);
        return outputData;
    }
    ...
    D3D12Texture::D3D12Texture(D3D12Device *device, D3D12Pool *pool, const RenderTextureDesc &desc) {
        ...
        wr64D3D12Failure(device->d3d, "CreateResource texture", res);
        if (FAILED(res)) {
            fprintf(stderr, "CreateResource failed with error code 0x%lX.\n", res);
            return;
        }
    ...
    uint64_t D3D12AccelerationStructure::getDeviceAddress() const {
        return buffer != nullptr ? buffer->d3d->GetGPUVirtualAddress() + offset : 0;
    }
...
#       endif
        ...
        const HRESULT result = device->d3d->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&d3d));
        wr64D3D12Failure(device->d3d, "CreateGraphicsPipelineState", result);
    }
...
#   endif
    ...
    void D3D12Device::setTopLevelASBuildInfo(RenderTopLevelASBuildInfo &buildInfo, const RenderTopLevelASInstance *instances, uint32_t instanceCount, bool preferFastBuild, bool preferFastTrace) {
        ...
        for (uint32_t i = 0; i < instanceCount; i++) {
            ...
            instanceDesc.AccelerationStructure = instance.bottomLevelASDeviceAddress != 0
                ? instance.bottomLevelASDeviceAddress
                : interfaceBottomLevelAS->d3d->GetGPUVirtualAddress() + instance.bottomLevelAS.offset;
