...
namespace RT64 {
    ...
    bool DebuggerInspector::checkPopup(Workload &workload) {
        ...
        if (ImGui::BeginPopup("##popupCallindices")) {
            for (const PopupHit &hit : popupCalls) {
                const CallIndices call = hit.call;
                ...
                if (hit.triangleIndex >= 0) {
                    ...
                    const uint32_t triangleCount =
                        proj.gameCalls[call.drawCallIndex].callDesc.triangleCount;
                    if (triangleCount == 1440U) {
                        callName += (hit.triangleIndex < 864) ?
                            " [native water]" : " [annulus]";
                    }
                }
    ...
    void DebuggerInspector::rightClick(const Workload &workload, hlslpp::float2 cursorPos) {
                ...
                if (proj.type == Projection::Type::Rectangle) {
                    for (uint32_t d = 0; d < proj.gameCallCount; d++) {
                        ...
                        if (callDesc.rect.contains(cursorRectX, cursorRectY)) {
                            ...
                            hits.push_back({ { f, p, d }, -1, fakeDepth });
                        }
        ...
        if (!hits.empty()) {
            ...
            std::sort(hits.begin(), hits.end(),
                [](const PopupHit &a, const PopupHit &b) {
                return a.depth < b.depth;
            });

            for (const PopupHit &hit : hits) {
                if (insertedIndices.find(hit.call) != insertedIndices.end()) {
                    continue;
                }

                popupCalls.push_back(hit);
                insertedIndices.insert(hit.call);
            }
