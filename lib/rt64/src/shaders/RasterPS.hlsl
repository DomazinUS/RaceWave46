    ...
    bool isFrontFace, out float4 resultColor, out float4 resultAlpha) 
{
    ...
    if (renderFlagUsesTexture0(rp.flags)) {
        ...
        texVal0 = sampleTexture(otherMode, rp.flags, textureUV, ddxVertexUV, ddyVertexUV, rdpTile, gpuTile, false,
            instanceRDPParams[instanceIndex].wr64UiFilter);
    }
    ...
    // The Dolphin disk's native combiner ignores primitive alpha. Fade the
    // combined alpha instead, before its existing alpha compare and blender.
    // No tag means the shader follows the original path without arithmetic.
    if (gConstants.padding.z == WR64_DOLPHIN_SUN_FADE_TAG) {
        const float opacity = saturate(1.0f - asfloat(gConstants.padding.y));
        if (opacity <= 0.0f) return false;
        combinerColor.a *= opacity;
        alphaCompareValue *= opacity;
    }
