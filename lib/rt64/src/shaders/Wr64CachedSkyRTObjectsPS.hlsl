// Independent mixed mode: validated RT objects with cached raster sky.
...
float wr64CachedSkyReflectionMultiplier() {
    const float value = wr64QualityOptions.z;
    return !isfinite(value) || value <= 0 ? 1.0f : clamp(value, 1.0f, 2.0f);
}

bool wr64CachedSkyColor(float3 direction, out float4 color, out Wr64SkyLookupDiagnostic diagnostic) {
    diagnostic = (Wr64SkyLookupDiagnostic)0;
    float4 premultiplied = wr64SampleRasterSky(direction);
    color = 0;
    if (!all(isfinite(premultiplied)) || premultiplied.a <= 0) return false;
    color = float4(premultiplied.rgb / premultiplied.a, premultiplied.a);
    return true;
}
...
void PSMain(float4 position : SV_POSITION, float2 uv : TEXCOORD0,
    ...
    if (hit) {
        ...
        skyHit = wr64CachedSkyColor(ray.Direction, skyColor, skyDiagnostic);
    ...
    if (hit) {
        ...
        float3 reflectedPremultiplied = reflected * reflectedAlpha;
        // Boost only the cached sky behind the unchanged translucent beams.
        // Object hits and their validated reflection/shadow response stay intact.
        const float multiplier = wr64CachedSkyReflectionMultiplier();
        if (multiplier > 1 && cachedSkyColor.a > 0) {
            const float extraWeight = (saturate(fresnel * wr64Strength * multiplier) -
                saturate(fresnel * wr64Strength)) * (1 - beams.a);
            reflectedAlpha += cachedSkyColor.a * extraWeight;
            reflectedPremultiplied += cachedSkyColor.rgb * cachedSkyColor.a * extraWeight;
        }
        ...
        if (reflectedAlpha > 0 && combinedAlpha > 0) {
            pixelColor.rgb = (reflectedPremultiplied + pixelColor.rgb * transmittedAlpha) / combinedAlpha;
