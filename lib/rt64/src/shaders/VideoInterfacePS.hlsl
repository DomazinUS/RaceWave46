...
float4 PSMain(in float4 pos : SV_Position, in float2 uv : TEXCOORD0) : SV_TARGET {
    const float2 sourceUV = gConstants.sourceUVOffset + uv * gConstants.sourceUVScale;
#ifdef PIXEL_ANTIALIASING
    return PixelAntialiasing(sourceUV);
#else
    return SampleInput((sourceUV / gConstants.textureResolution) * gConstants.videoResolution);
#endif
}
