...
#ifdef __spirv__
// Match Workload's native byte and 16-bit index views. DXC otherwise derives
// 32-bit texel formats from the HLSL scalar types, making these reads undefined.
...
StructuredBuffer<uint4> wr64WaterFieldInfo : register(t22);
StructuredBuffer<float4> wr64WaterFieldPrevPosShiftX : register(t23);
StructuredBuffer<float4> wr64WaterFieldPrevTcShiftZ : register(t24);
StructuredBuffer<uint> wr64WaterFieldFaceIndices : register(t25);
...
void CSMain(uint vertexIndex : SV_DispatchThreadID) {
    ...
    // NDC Position.
    float3 pos = float3(srcPos[posIndex + 0], srcPos[posIndex + 1], srcPos[posIndex + 2]);
    const float3 vel = float3(srcVel[posIndex + 0], srcVel[posIndex + 1], srcVel[posIndex + 2]);
    pos -= vel * (1.0f - gConstants.curFrameWeight);
    float2 wr64EvaluatedTc = 0.0f;
    bool wr64EvaluatedTcValid = false;

#define WR64_WATER_FRAME_WEIGHT gConstants.curFrameWeight
#include "Wr64WaterVertexBody.hlsli"
#undef WR64_WATER_FRAME_WEIGHT

    float4 tfPos = mul(mul(viewProjMat, worldMat), float4(pos, 1.0f));
