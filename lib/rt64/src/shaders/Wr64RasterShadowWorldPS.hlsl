...
void PSMain(float4 position : SV_POSITION, float2 uv : TEXCOORD0,
    ...
    if (wr64ShadowMapLight.w != 1) return;
    ...
    // vertex-test rewrites), so SV_PrimitiveID identifies the exact native face.
    ...
    const float tangentTolerance = (8.0f * 1.1920928955078125e-7f) * coordinateScale *
        sqrt(longestEdgeSquared) * inverseNormalLength * length(wr64ShadowMapLight.xyz);
    if (!isfinite(tangentTolerance)) return;
    if (dot(normal, wr64ShadowMapLight.xyz) < -tangentTolerance) normal = -normal;
    ...
    float shadowCoverage = wr64RasterShadowCoverage(receiverPosition, normal, false);
    if (wr64DebugView == 5) {
        pixelColor.rgb = 1 - shadowCoverage;
        pixelAlpha.a = 1;
        return;
    }
    const float transmission = 1 - clamp(wr64ShadowMapOptions.w, 0.0f, 0.55f) * saturate(shadowCoverage);
    if (transmission < 1) {
