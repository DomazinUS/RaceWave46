...
float4 sampleTexture(OtherMode otherMode, RenderFlags renderFlags, float2 inputUV, float2 ddxUV, float2 ddyUV, const RDPTile rdpTile, const GPUTile gpuTile, bool nextPixelBug, uint wr64UiFilter = 0) {
    ...
    // The caller enables this only for identified Wave Race HUD/menu draws.
    // Mode zero reaches the unmodified RDP sampler below. Copy/framebuffer,
    // replacement, raw-TMEM and minified textures retain their normal path.
    if (wr64UiFilter != 0) {
        const float2 derivativeScale = perspCorrectionMod * float2(rdpTile.shifts, rdpTile.shiftt) * gpuTile.tcScale;
        const float2 footprint = abs(ddxUV * derivativeScale) + abs(ddyUV * derivativeScale);
        if (wr64UiCanFilter(wr64UiFilter, otherMode, gpuTile, footprint)) {
            // Point-filtered UVs identify a texel's interval; bilinear UVs
            // identify its center. Preserve that half-texel distinction.
            const float2 texelCoord = uvCoord - ((!filterBilerp && !linearFiltering) ? 0.5f : 0.0f);
            return (wr64UiFilter == 1) ? wr64UiLight(rdpTile, gpuTile, texelCoord, tlut)
                : wr64UiEnhanced(rdpTile, gpuTile, texelCoord, tlut);
        }
    }
