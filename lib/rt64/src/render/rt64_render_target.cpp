...
namespace RT64 {
    ...
    void RenderTarget::setupColor(RenderWorker *worker, uint32_t width, uint32_t height) {
        ...
        WR64StallProbe stallProbe("render-target-setup-color", uint64_t(width) << 32 | uint64_t(height));
        releaseTextures();
    ...
    void RenderTarget::setupDepth(RenderWorker *worker, uint32_t width, uint32_t height) {
        ...
        WR64StallProbe stallProbe("render-target-setup-depth", uint64_t(width) << 32 | uint64_t(height));
        releaseTextures();
    ...
    void RenderTarget::copyFromChanges(RenderWorker *worker, const FramebufferChange &fbChange, uint32_t fbWidth, uint32_t fbHeight, uint32_t rowStart, const ShaderLibrary *shaderLibrary) {
        ...
        //
        // WR64_CPU_CHANGE_WIDE (EXP-017): on aspect-expanded targets (resolutionScale.x >
        // resolutionScale.y) the stock path below reconstructs CPU-written framebuffer
        // changes with a native-aspect, horizontally centered mapping (fbWidth *
        // resolutionScale.y wide). GPU draws on the same target span the full expanded
        // width (fbWidth * resolutionScale.x), so every CPU-composited layer falls short
        // by (x-y)*fbWidth/2 pixels per side -- the asymmetric bright/dark edge strips on
        // the WR64 menus. With the gate enabled, map CPU changes across the full expanded
        // width, anchored at 0, exactly like GPU geometry. When resolutionScale.x ==
        // resolutionScale.y (4:3 window, scratch {3,3} targets, boot logo) this mapping
        // is identical to the stock path, so those screens are unaffected.
        // ★ VALIDATED FIX, COMPILED IN. No environment variable.
        constexpr bool cpuChangeWideEnabled = true;
        float targetWidth, targetHeight, targetLeft, targetTop;
        const bool pillarBox = resolutionScale[0] > resolutionScale[1];
        if (cpuChangeWideEnabled) {
        ...
        // WR64_CPU_CHANGE_TRACE (diagnostic): log every CPU-change composition so we
        // can confirm whether this path runs at all on the affected screens and with
        // what scale/mapping.
        {
            static const char *cpuChangeTraceValue = std::getenv("WR64_CPU_CHANGE_TRACE");
            static const bool cpuChangeTraceEnabled = (cpuChangeTraceValue != nullptr) &&
                (cpuChangeTraceValue[0] == '1') && (cpuChangeTraceValue[1] == '\0');
            if (cpuChangeTraceEnabled) {
                static int wr64CpuChangeLogged = 0;
                if (wr64CpuChangeLogged < 512) {
                    wr64CpuChangeLogged++;
                    std::printf("[cpuchange] fb=%ux%u rowStart=%u targetScale=(%g,%g) wideGate=%d out=(%g,%g %gx%g) depth=%d\n",
                        fbWidth, fbHeight, rowStart, double(resolutionScale.x), double(resolutionScale.y),
                        cpuChangeWideEnabled ? 1 : 0, double(targetLeft), double(targetTop), double(targetWidth), double(targetHeight),
                        (format == depthBufferFormat()) ? 1 : 0);
                    std::fflush(stdout);
                }
            }
        }
