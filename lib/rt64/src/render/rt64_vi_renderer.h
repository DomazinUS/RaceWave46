...
namespace RT64 {
    struct VIRenderer {
        ...
        struct RenderParams {
            ...
            // The render target may round its width for allocation/alignment. Keep
            // that scale for texture sampling, but use the unrounded scale below
            // when placing the final VI image in the host window.
            hlslpp::float2 resolutionScale;
            hlslpp::float2 presentationResolutionScale;
            ...
            Wr64PresentRegion outputRegion;
        };
