...
namespace RT64 {
    ...
    void ProjectionProcessor::processScene(const ProcessParams &p, const GameScene &scene, size_t sceneIndex) {
        for (size_t i = 0; i < scene.projections.size(); i++) {
            ...
            // Match the USA renderer's validated full-scissor viewport rule.
            // Menu slide/scroll viewports can be offset and their matrices can
            // classify as Perspective. Their authored 424-wide geometry must
            // retain its scale just like gameplay when the viewport expands.
            // Inset craft previews keep the generic centred-viewport behavior.
            if (p.aspectRatioScale > 1.000001f &&
            ...
            if (p.aspectRatioScale > 1.000001f &&
                (proj.wr64MenuModelPreview || wr64WatercraftSelectFramebuffer(workload, sceneProj.fbPairIndex))) {
                // Native result/ranking and Watercraft Select thumbnail centres belong to authored
                // viewports. Widening their centres while shrinking only their
                // projection moves the craft outside the unchanged UI boxes.
                // The renderer centres this complete menu plane, as in the
                // existing Shindou host implementation. Its measured backdrop
                // is independently given full target coverage per draw.
            ...
            if ((prevProjMatrix != nullptr) && (prevViewMatrix != nullptr) && (rigidBody != nullptr)) {
                ...
                const bool interpolateProjection = (rigidBody->lerpTranslation || rigidBody->lerpRotation) &&
                    (!wr64ProjectionGuardEnabled() || wr64ProjectionsEffectivelyIdentical(adjustedPrevProj, curProjTransform));
