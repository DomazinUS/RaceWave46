...
namespace RT64 {
    ...
    void BufferUploader::threadLoop() {
        ...
        wr64StallProbeNameThread("buffer-upload");
    ...
    void BufferUploader::updateResources(RenderWorker *worker, std::vector<Upload> &blankUploads) {
        for (Upload &u : blankUploads) {
            ...
            // Two createBuffer calls plus one formatted view per format, and then the
            // whole buffer is reuploaded from index zero because the storage moved.
            // Grows by 1.5x so it should settle early, but a course that suddenly
            // needs more geometry than anything before it pays for it mid race.
            WR64StallProbe regrowProbe("buffer-pair-regrow", uint64_t(requiredSize));
    ...
    void BufferUploader::wait() {
        // The draw thread parked until the buffer thread has finished memcpying this
        // frame's geometry. Should be short. If it is not, the regrow above is the
        // usual reason and both will appear together in the dump.
        WR64StallProbeHot waitProbe("buffer-upload-wait");
        std::unique_lock<std::mutex> readyLock(readyMutex);
