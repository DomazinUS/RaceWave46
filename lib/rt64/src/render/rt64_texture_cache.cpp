...
namespace RT64 {
    ...
    bool ReplacementMap::saveDatabase(std::ostream &stream) {
        try {
            ...
            if (!fileSystemDolphinCaches.empty() && fileSystemDolphinCaches.front().enabled) {
                jroot["configuration"]["wr64DolphinCompatible"] = true;
            }
    ...
    void TextureCache::uploadThreadLoop() {
        ...
        wr64StallProbeNameThread("texture-upload");
        ...
        while (uploadThreadRunning) {
            ...
            if (!queueCopy.empty() || !resolvedPathQueueCopy.empty() || !afterDecodeBarriers.empty()) {
                ...
                // Whole cohort. The draw thread blocks on this batch through
                // waitForGPUUploads, so its total is the ceiling on how long a frame
                // can be held up by texture work. detail is the cohort size.
                WR64StallProbe uploadBatchProbe("texture-upload-batch", uint64_t(queueSize));
                ...
                // One createTexture plus one setName string build per texture in the
                // cohort. The name is a heap string concatenation that the release
                // build still performs and the driver still copies.
                {
                WR64StallProbe tmemCreateProbe("texture-create-tmem", uint64_t(queueSize));
                ...
                // Second createTexture plus setName per decoded texture, and the
                // replacement path resolution for every one of them.
                {
                WR64StallProbe decodeSetupProbe("texture-decode-setup", uint64_t(queueSize));
                for (size_t i = 0; i < queueSize; i++) {
                    const TextureUpload &upload = queueCopy[i];
                ...
                // Replacement resolution. Contains a synchronous file system load on
                // the Stall path, which is the one blocking disk read left in here.
                {
                WR64StallProbe replacementProbe("texture-replacement-resolve", uint64_t(resolvedPathQueueCopy.size()));
...
#                           endif
                ...
                // Hard CPU block on the GPU for the whole cohort. Nothing else in
                // this batch can overlap it and the draw thread is behind it.
                {
                    WR64StallProbe gpuWaitProbe("texture-upload-gpu-wait", uint64_t(queueSize));
                    directWorker->wait();
                }

                // Delete all the resources used during the upload of replacements.
                {
                    WR64StallProbe freeProbe("texture-replacement-free", uint64_t(replacementUploadResources.size()));
                    replacementUploadResources.clear();
                }

                // Add all the textures to the map once they're ready.
                {
                    WR64StallProbe commitProbe("texture-map-commit",
                        uint64_t(textureMapAdditions.size() + replacementMapAdditions.size()));
                    std::unique_lock lock(textureMapMutex);
                ...
                {
                    WR64StallProbe rebuildProbe("texture-queue-rebuild", uint64_t(queueSize));
                    std::unique_lock queueLock(uploadQueueMutex);
    ...
    void TextureCache::waitForGPUUploads() {
        WR64StallProbe stallProbe("texture-upload-wait");
        std::unique_lock queueLock(uploadQueueMutex);
    ...
    void TextureCache::addResolvedPaths(uint64_t hash, uint32_t width, uint32_t height, uint32_t tlut, const LoadTile &loadTile, const std::vector<uint8_t> &bytesTMEM, bool decodeTMEM, std::vector<ReplacementResolvedPath> &resolvedPaths, uint64_t exclusiveDbHash) {
        ...
        for (int32_t i = int32_t(textureMap.replacementMap.fileSystemResolvedPaths.size()) - 1; i >= 0; i--) {
            ...
            if ((exclusiveDbHash == 0) || (exclusiveDbHash == hashes[hashVersion])) {
                ...
                // A pack explicitly opting into Dolphin compatibility can match
                // new native textures without an offline RT64 hash entry. Keep
                // this inside the filesystem priority loop: a higher-priority
                // dynamic match must win over a lower-priority static entry.
                auto &dolphinCaches = textureMap.replacementMap.fileSystemDolphinCaches;
                if (!decodeTMEM || (bytesTMEM.size() != 0x1000U) || (size_t(i) >= dolphinCaches.size())) {
                    continue;
                }

                auto &dolphin = dolphinCaches[i];
                if (!dolphin.enabled || (dolphin.index.size() == 0)) {
                    continue;
                }

                auto cached = dolphin.resolvedPaths.find(hash);
                if (cached != dolphin.resolvedPaths.end()) {
                    resolvedPaths.emplace_back(cached->second);
                    break;
                }
                if (dolphin.missingHashes.find(hash) != dolphin.missingHashes.end()) {
                    continue;
                }

                const std::string relativePath = dolphin.index.match(bytesTMEM.data(), bytesTMEM.size(), width, height, tlut, loadTile);
                if (relativePath.empty()) {
                    dolphin.missingHashes.insert(hash);
                }
                else {
                    ReplacementResolvedPath resolvedPath{};
                    resolvedPath.fileSystemIndex = uint32_t(i);
                    resolvedPath.textureHash = hash;
                    resolvedPath.relativePath = relativePath;
                    resolvedPath.originalOperation = ReplacementOperation::Preload;
                    resolvedPath.resolvedOperation = ReplacementOperation::Preload;
                    resolvedPath.originalShift = ReplacementShift::Auto;
                    resolvedPath.resolvedShift = dolphin.pathShifts.at(relativePath);
                    dolphin.resolvedPaths.emplace(hash, resolvedPath);
                    resolvedPaths.emplace_back(std::move(resolvedPath));
                }

                const size_t lookupCount = dolphin.resolvedPaths.size() + dolphin.missingHashes.size();
                if ((lookupCount <= 8) || ((lookupCount & (lookupCount - 1)) == 0)) {
                    fprintf(stderr, "[textures] Dolphin pack %u: %zu exact native matches, %zu unmatched native textures.\n",
                        uint32_t(i), dolphin.resolvedPaths.size(), dolphin.missingHashes.size());
                }
                if (!relativePath.empty()) {
                    break;
                }
    ...
    bool TextureCache::useTexture(uint64_t hash, uint64_t submissionFrame, uint32_t &textureIndex, interop::float2 &textureScale, interop::float3 &textureDimensions, bool &textureReplaced, bool &hasMipmaps, bool &shiftedByHalf) {
        // Draw path. This is the other side of every textureMapMutex hold in the
        // upload thread, so if a cohort commit is what stalls the frame, this is
        // where the stall is actually paid. Hot probe: silent unless it blocks.
        WR64StallProbeHot useProbe("texture-map-lock-use");
        std::unique_lock lock(textureMapMutex);
    ...
    bool TextureCache::loadReplacementDirectories(const std::vector<ReplacementDirectory> &replacementDirectories) {
        ...
        std::vector<ReplacementMap::DolphinCache> fileSystemDolphinCaches;
        ...
        fileSystemDolphinCaches.resize(fileSystems.size());
            ...
            for (int32_t i = int32_t(fileSystems.size()) - 1; i >= 0; i--) {
                if (fileSystems[i]->load(ReplacementDatabaseFilename, databaseBytes)) {
                    try {
                        ...
                        const json databaseJson = json::parse(databaseBytes.begin(), databaseBytes.end(), nullptr, true);
                        db = databaseJson;
                        ...
                        if (db.config.hashVersion <= TMEMHasher::CurrentHashVersion) {
                            ...
                            const auto configuration = databaseJson.find("configuration");
                            const bool dolphinCompatible = (configuration != databaseJson.end()) && configuration->is_object() &&
                                configuration->value("wr64DolphinCompatible", false);
                            if (dolphinCompatible && (db.config.defaultOperation == ReplacementOperation::Preload)) {
                                auto &dolphin = fileSystemDolphinCaches[i];
                                dolphin.enabled = true;
                                for (const std::string &path : *fileSystems[i]) {
                                    const std::filesystem::path pathFs = std::filesystem::u8path(path);
                                    const auto extension = pathFs.extension().u8string();
                                    if ((extension != u8".dds") && (extension != u8".DDS")) {
                                        continue;
                                    }
                                    const std::string relativePath = FileSystem::toForwardSlashes(path);
                                    dolphin.index.addPath(relativePath);
                                    dolphin.pathShifts.emplace(relativePath, db.resolveShift(relativePath, ReplacementShift::Auto));
                                    // Dynamic matches have no native hash yet, so
                                    // preload all DDS candidates through RT64's
                                    // existing stream workers when the pack loads.
                                    fileSystemStreamSets[i].insert(relativePath);
                                }
                                fprintf(stderr, "[textures] Dolphin pack %u indexed %zu texture names.\n", uint32_t(i), dolphin.index.size());
                            }
                            else if (dolphinCompatible) {
                                fprintf(stderr, "[textures] Dolphin compatibility requires defaultOperation=preload; native mappings remain available.\n");
                            }
        ...
        textureMap.replacementMap.fileSystemDolphinCaches = std::move(fileSystemDolphinCaches);
    ...
    bool TextureCache::evict(uint64_t submissionFrame, std::vector<uint64_t> &evictedHashes) {
        WR64StallProbe stallProbe("texture-evict");
        std::unique_lock lock(textureMapMutex);
        const bool evicted = textureMap.evict(submissionFrame, evictedHashes);
        stallProbe.setDetail(evictedHashes.size());
        return evicted;
    }
    ...
    void TextureCache::decrementLock() {
        ...
        if (lockCounter == 0) {
            ...
            //
            // These are retired in bounded batches rather than all at once. The
            // eviction policy ages textures out relative to how long they have
            // already been alive, so a set of textures that was created together
            // and stops being drawn together also falls out of the access list
            // together. Destroying a whole cohort in a single call releases every
            // backing GPU allocation at the same moment, while holding
            // textureMapMutex, which the draw path needs for useTexture. That is a
            // one-off multi frame stall a couple of times through a course rather
            // than a steady cost, which is why it does not show up as a frame rate
            // drop and does show up as a hitch.
            //
            // Deferring the remainder is safe: everything in this vector has
            // already been removed from the texture map and is unreferenced, so
            // holding it a few more frames costs memory and nothing else. This runs
            // once per rendered frame, so at 180 Hz the drain rate is far above any
            // rate the cache can evict at, and the destructor releases whatever is
            // still outstanding at shutdown.
            constexpr size_t MaxRetiredPerCall = 8;
            const size_t pendingCount = textureMap.evictedTextures.size();
            const size_t retireCount = (pendingCount < MaxRetiredPerCall) ? pendingCount : MaxRetiredPerCall;
            if (retireCount > 0) {
                WR64StallProbe stallProbe("texture-retire", retireCount);
                for (size_t i = 0; i < retireCount; i++) {
                    delete textureMap.evictedTextures[i];
                }

                textureMap.evictedTextures.erase(textureMap.evictedTextures.begin(),
                    textureMap.evictedTextures.begin() + retireCount);
            }
        }
    }
