...
namespace RT64 {
    ...
    void VertexProcessor::process(const ProcessParams &p) {
        ...
        worldCB.wr64RtWater = p.wr64RtWater ? 1U : 0U;
        ...
        descriptorSet->setBuffer(descriptorSet->srcTc, p.drawBuffers->texcoordBuffer.get(), p.drawBuffers->texcoordBuffer.getView(0));
        descriptorSet->setBuffer(descriptorSet->srcTcVel, p.drawBuffers->texcoordVelocityBuffer.get(), p.drawBuffers->texcoordVelocityBuffer.getView(0));
        descriptorSet->setBuffer(descriptorSet->wr64WaterFieldInfo, p.drawBuffers->wr64WaterFieldInfoBuffer.get(), RenderBufferStructuredView(sizeof(uint32_t) * 4));
        descriptorSet->setBuffer(descriptorSet->wr64WaterFieldPrevPosShiftX, p.drawBuffers->wr64WaterFieldPrevPosShiftXBuffer.get(), RenderBufferStructuredView(sizeof(float) * 4));
        descriptorSet->setBuffer(descriptorSet->wr64WaterFieldPrevTcShiftZ, p.drawBuffers->wr64WaterFieldPrevTcShiftZBuffer.get(), RenderBufferStructuredView(sizeof(float) * 4));
        descriptorSet->setBuffer(descriptorSet->wr64WaterFieldFaceIndices, p.drawBuffers->wr64WaterFieldFaceIndicesBuffer.get(), RenderBufferStructuredView(sizeof(uint32_t)));
    }
