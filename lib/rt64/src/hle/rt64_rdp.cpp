...
#include "common/rt64_wr64_ending_rect.h"
...
namespace RT64 {
    // WR64_FRAME_EDGE_SNAP (EXP-027): the IPS-native game authors "full frame"
    // coordinates inclusively as 423/239 (one native pixel short of the 424x240
    // logical frame), at EVERY level - scissors included - so downstream
    // frame-edge snapping in the renderer can only snap to an already-short
    // fence. Under this gate, snap incoming scissor/fillrect/texrect lower-right
    // coordinates onto the frame edge at command decode time, before anything
    // derives from them. Tolerance: two native pixels. 10.2 fixed point.
    // ★ VALIDATED FIX, COMPILED IN. No environment variable.
    static bool wr64FrameEdgeSnapEnabled() {
        return true;
    }

    static void wr64SnapLowerRight(int32_t &lrx, int32_t &lry) {
        const int32_t FrameW = 424 * 4, FrameH = 240 * 4, Tol = 8;
        if ((lrx < FrameW) && (lrx >= FrameW - Tol)) { lrx = FrameW; }
        if ((lry < FrameH) && (lry >= FrameH - Tol)) { lry = FrameH; }
    }
...
#   endif
    ...
    void RDP::setScissor(uint8_t mode, int32_t ulx, int32_t uly, int32_t lrx, int32_t lry, const ExtendedAlignment &extAlignment) {
        // WR64 two-player frame R1. The six split-screen scissor producers were
        // never reached by the one-player widescreen hooks, so the split-screen
        // play area is letterboxed inside a 408x217 inset of the 424x240 frame.
        // Corrected here, at command decode, before anything downstream sees it -
        // the same seam wr64SnapLowerRight already uses. Exact four-edge match on
        // three authored rects; see common/rt64_wr64_2p_frame.h for the ROM sites
        // and the proof that no one-player path can produce them.
        const int32_t wr64AuthoredUlx = ulx;
        const int32_t wr64AuthoredUly = uly;
        const int32_t wr64AuthoredLrx = lrx;
        const int32_t wr64AuthoredLry = lry;
        const bool wr64TwoPCorrected = (wr64TwoPFrameCorrectScissor(ulx, uly, lrx, lry) != nullptr);
        wr64TwoPFrameRecord(wr64AuthoredUlx, wr64AuthoredUly, wr64AuthoredLrx, wr64AuthoredLry, wr64TwoPCorrected);

        if (wr64FrameEdgeSnapEnabled()) { wr64SnapLowerRight(lrx, lry); }
    ...
    void RDP::fillRect(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry, const ExtendedAlignment &extAlignment) {
        const bool wr64EndingPacket = state->wr64HudCurrent.kind == 3 && state->wr64HudCurrent.generation != 0;
        if (wr64FrameEdgeSnapEnabled() && !wr64EndingPacket) { wr64SnapLowerRight(lrx, lry); }
...
#   endif
    ...
    void RDP::clearExtended() {
        ...
        // WR64: the IPS-native game authors every rectangle in the wide
        // 424x240 logical space. With WR64_RECT_WIDE_SPACE=1 the persistent
        // rect-aspect state defaults to STRETCH (equivalent to the game
        // issuing gEXSetRectAspect at boot), so rects scale with the frame
        // instead of being aspect-corrected toward center.
        // ★ VALIDATED FIX, COMPILED IN. No environment variable.
        extended.global.rectAspect = G_EX_ASPECT_STRETCH;
    }
    
    void RDP::drawTris(uint32_t triCount, const float *pos, const float *tc, const float *col, uint8_t tile, uint8_t levels) {
        // WR64_QUAD_TRACE (EXP-025 diagnostic): log the coordinate extents of every
        // raw-triangle batch so the producer of the 423-wide steady backdrop layer
        // can be identified. Dedup per unique extent tuple.
        {
            static const char *wr64QuadTraceValue = std::getenv("WR64_QUAD_TRACE");
            static const bool wr64QuadTraceEnabled = (wr64QuadTraceValue != nullptr) &&
                (wr64QuadTraceValue[0] == '1') && (wr64QuadTraceValue[1] == '\0');
            if (wr64QuadTraceEnabled && (pos != nullptr) && (triCount > 0)) {
                float minX = 1e9f, maxX = -1e9f, minY = 1e9f, maxY = -1e9f;
                const uint32_t vtxCount = triCount * 3;
                for (uint32_t i = 0; i < vtxCount; i++) {
                    const float x = pos[i * 4 + 0]; const float y = pos[i * 4 + 1];
                    if (x < minX) minX = x; if (x > maxX) maxX = x;
                    if (y < minY) minY = y; if (y > maxY) maxY = y;
                }
                static std::set<uint64_t> wr64QuadSeen;
                const uint64_t key = (uint64_t(uint16_t(int(minX * 4))) << 48) | (uint64_t(uint16_t(int(maxX * 4))) << 32) |
                    (uint64_t(uint16_t(int(minY * 4))) << 16) | uint64_t(uint16_t(int(maxY * 4))) ^ (uint64_t(triCount) << 60);
                if ((wr64QuadSeen.size() < 4096) && wr64QuadSeen.insert(key).second) {
                    std::printf("[quad] tris=%u x=%g..%g y=%g..%g\n",
                        triCount, double(minX), double(maxX), double(minY), double(maxY));
                    std::fflush(stdout);
                }
            }
        }
    ...
    void RDP::drawRect(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry, int16_t uls, int16_t ult, int16_t dsdx, int16_t dtdy, bool flip, const ExtendedAlignment &extAlignment) {
        // Moving ending panels retain their authored bounds across the edge;
        // per-strip frame snapping would stretch only a subset of the panel.
        const bool wr64EndingPacket = state->wr64HudCurrent.kind == 3 && state->wr64HudCurrent.generation != 0;
        const auto &wr64EndingCombiner = colorCombinerStack[colorCombinerStackSize - 1];
        const bool wr64EndingTopRestored = wr64EndingRestoreTop(state->wr64HudCurrent,
            wr64EndingCombiner.H, wr64EndingCombiner.L, otherMode.H, otherMode.L,
            extAlignment.leftOrigin == G_EX_ORIGIN_NONE && extAlignment.rightOrigin == G_EX_ORIGIN_NONE &&
            extAlignment.leftOffset == 0 && extAlignment.topOffset == 0 &&
            extAlignment.rightOffset == 0 && extAlignment.bottomOffset == 0,
            flip, ulx, uly, lrx, lry, ult, dsdx, dtdy);
        const auto wr64DolphinSprite = state->wr64DolphinSunCurrent;
        state->wr64DolphinSunCurrent = {};
        const bool wr64DolphinPacket = wr64DolphinSunSpriteValid(wr64DolphinSprite) && !flip &&
            dsdx == wr64DolphinSprite.dsdx && dtdy == wr64DolphinSprite.dtdy &&
            extAlignment.leftOrigin == G_EX_ORIGIN_NONE && extAlignment.rightOrigin == G_EX_ORIGIN_NONE &&
            extAlignment.leftOffset == 0 && extAlignment.topOffset == 0 &&
            extAlignment.rightOffset == 0 && extAlignment.bottomOffset == 0 &&
            otherMode.cycleType() != G_CYC_FILL && otherMode.cycleType() != G_CYC_COPY;
        const bool wr64DolphinEmptyCarrier = wr64DolphinPacket && wr64DolphinSprite.centerY < 0.0f &&
            ulx < lrx && uly == 24 && lry == 24;
        if (wr64FrameEdgeSnapEnabled() && !wr64EndingPacket) { wr64SnapLowerRight(lrx, lry); }
        ...
        const FixedRect &scissorRect = state->rdp->scissorRectStack[scissorStackSize - 1];
        const bool fixRectLR = state->ext.enhancementConfig->rect.fixRectLR && !wr64EndingPacket;
        ...
        if (drawRect.isEmpty() && !wr64DolphinEmptyCarrier) {
            return;
        }
        ...
        drawCall.wr64HudMotion = state->wr64HudCurrent;
        state->wr64HudCurrent = {};
        drawCall.wr64DolphinSunSprite = wr64DolphinPacket ? wr64DolphinSprite : Wr64DolphinSunSprite{};
        ...
        if (wr64DolphinPacket) {
            // The final presentation samples the complete loaded DP texture.
            // Retain native UV vertices; only widen the authenticated sampling
            // inventory so an empty carrier still resolves its full tile.
            const auto fullUV = wr64DolphinSunFullUV(wr64DolphinSprite);
            updateCallTexcoords(fullUV[0], fullUV[1]);
            updateCallTexcoords(fullUV[2], fullUV[3]);
        }
        if (wr64EndingTopRestored) {
            // Interpolation may expose rows above the native clipped top.
            // Keep the complete restored carrier in the sampling inventory.
            updateCallTexcoords(u1, v1);
            updateCallTexcoords(u2, v2);
        }
    }
    ...
    void RDP::drawTexRect(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry, uint8_t tile, int16_t uls, int16_t ult, int16_t dsdx, int16_t dtdy, bool flip, const ExtendedAlignment &extAlignment) {
        const bool wr64EndingPacket = state->wr64HudCurrent.kind == 3 && state->wr64HudCurrent.generation;
        if (wr64FrameEdgeSnapEnabled() && !wr64EndingPacket) { wr64SnapLowerRight(lrx, lry); }
