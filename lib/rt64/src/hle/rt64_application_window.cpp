...
#   endif
    ...
    void ApplicationWindow::windowMessage(UINT message, WPARAM wParam, LPARAM lParam) {
        const std::lock_guard<std::recursive_mutex> lock(eventFilterMutex);
        if (eventFiltersStopped) return;
    ...
    LRESULT CALLBACK ApplicationWindow::windowHookCallback(int nCode, WPARAM wParam, LPARAM lParam) {
        if ((nCode >= 0) && (wParam == PM_REMOVE)) {
            ...
            if (HookedApplicationWindow != nullptr) {
    ...
    void ApplicationWindow::stopEventFilters() {
        {
            // Wait for a listener already running on another thread before
            // Application::end releases anything that listener can access.
            const std::lock_guard<std::recursive_mutex> lock(eventFilterMutex);
            eventFiltersStopped = true;
        }

        // Installation/removal belong to the application/render thread. Never
        // hold our callback mutex while entering SDL's filter registration.
        if (sdlEventFilterInstalled) {
            ...
            if (SDL_GetEventFilter(&currentFilter, &currentUserdata) &&
                currentFilter == &ApplicationWindow::sdlEventFilter && currentUserdata == this) {
    ...
    int ApplicationWindow::sdlEventFilter(void *userdata, SDL_Event *event) {
        ...
        {
            const std::lock_guard<std::recursive_mutex> lock(appWindow->eventFilterMutex);
            if (!appWindow->eventFiltersStopped && appWindow->listener->sdlEventFilter(event)) {
                return 0;
            }
            previousFilter = appWindow->sdlEventFilterStored;
            previousUserdata = appWindow->sdlEventFilterUserdata;
        }
        ...
        if (previousFilter != nullptr) {
            return previousFilter(previousUserdata, event);
        }
