...
#       endif
        ...
        WR64_DIAGNOSTIC_LOG(fprintf(stdout, "Device Name: %s\n", deviceDescription.name.c_str()););
        WR64_DIAGNOSTIC_LOG(fprintf(stdout, "Device Vendor: 0x%X\n", deviceDescription.vendor););
        WR64_DIAGNOSTIC_LOG(fprintf(stdout, "Driver Version: 0x%" PRIx64 "\n", deviceDescription.driverVersion););
...
#       endif
    ...
    void Application::destroyShaderCache() {
        ...
        // These optional RT pipelines compile on their own workers. Their
        // layout belongs to shaderUber and must outlive all pending driver work.
        workloadQueue->framebufferRenderer->resetWr64RTPipelines();
