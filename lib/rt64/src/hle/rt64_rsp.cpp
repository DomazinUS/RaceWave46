...
namespace RT64 {
    ...
    struct Wr64ArticulatedMatrixInfo {
        bool valid = false;
        uint32_t racerIndex = 0;
        uint32_t limbIndex = 0;
    };

    static Wr64ArticulatedMatrixInfo decodeWr64ArticulatedMatrixAddress(uint32_t segmentedAddress) {
        const uint32_t segment = segmentedAddress & 0xFF000000;
        const uint32_t offset = segmentedAddress & 0x00FFFFFF;
        if ((segment != 0x03000000) || (offset < 0x0000E108) || (offset > 0x0000F3C8)) {
            return {};
        }

        const uint32_t relativeOffset = offset - 0x0000E108;
        const uint32_t racerOffset = relativeOffset & 0xFF;
        if ((racerOffset & 0x3F) != 0) {
            return {};
        }

        Wr64ArticulatedMatrixInfo result;
        result.valid = true;
        result.racerIndex = racerOffset >> 6;
        result.limbIndex = relativeOffset >> 8;
        return result;
    }

    static uint32_t wr64ArticulatedMatrixId(const Wr64ArticulatedMatrixInfo &articulated, uint32_t viewportAddress) {
        // Each split view needs its own history. If P1 starts seeing P2's racer,
        // a shared ID lets that new copy consume the only previous transform,
        // leaving P2's continuing copy without interpolation. Native P2 world
        // draws use this viewport address in both horizontal and vertical layout.
        const uint32_t viewId = (viewportAddress == 0x800DA8E0U) ? 0x00010000U : 0U;
        return 0x57000000U | viewId | (articulated.racerIndex << 8) | articulated.limbIndex;
    }

    struct Wr64SelectorMatrixInfo {
        bool valid = false;
        uint32_t watercraftIndex = 0;
        uint32_t componentIndex = 0;
    };

    static Wr64SelectorMatrixInfo decodeWr64SelectorMatrixAddress(uint32_t physicalAddress) {
        constexpr uint32_t BufferAddresses[] = { 0x001CC348, 0x001CDAE0 };
        constexpr uint32_t MatrixStride = 0x40;
        constexpr uint32_t ComponentsPerWatercraft = 3;
        constexpr uint32_t MatrixCount = 4 * ComponentsPerWatercraft;

        for (uint32_t bufferAddress : BufferAddresses) {
            const uint32_t byteCount = MatrixCount * MatrixStride;
            if ((physicalAddress < bufferAddress) || (physicalAddress >= (bufferAddress + byteCount))) {
                continue;
            }

            const uint32_t relativeAddress = physicalAddress - bufferAddress;
            if ((relativeAddress % MatrixStride) != 0) {
                return {};
            }

            const uint32_t slot = relativeAddress / MatrixStride;
            Wr64SelectorMatrixInfo result;
            result.valid = true;
            result.watercraftIndex = slot / ComponentsPerWatercraft;
            result.componentIndex = slot % ComponentsPerWatercraft;
            return result;
        }
    ...
    static bool isWr64CourseSelectorMapMatrixAddress(uint32_t physicalAddress) {
        return (physicalAddress == 0x001CC148) || (physicalAddress == 0x001CD8E0);
    }

    static TransformGroup makeWr64FinalMatrixGroup(uint32_t matrixId, bool interpolateGeneratedVertices) {
        TransformGroup group;
        group.matrixId = matrixId;
        ...
        group.scaleInterpolation = G_EX_COMPONENT_INTERPOLATE;
        group.skewInterpolation = G_EX_COMPONENT_INTERPOLATE;
        group.perspectiveInterpolation = G_EX_COMPONENT_INTERPOLATE;
        group.vertexInterpolation = interpolateGeneratedVertices ? G_EX_COMPONENT_INTERPOLATE : G_EX_COMPONENT_SKIP;
        group.texcoordInterpolation = G_EX_COMPONENT_SKIP;
        group.tileInterpolation = G_EX_COMPONENT_INTERPOLATE;
        group.lookAtInterpolation = G_EX_COMPONENT_AUTO;
...
#   endif
    ...
    void RSP::setVertex(uint32_t address, uint32_t vtxCount, uint32_t dstIndex) {
        ...
        setVertexCommon<true, sizeof(Vertex)>(address, rdramAddress, dstIndex, dstIndex + vtxCount);
    }
    ...
    void RSP::setVertexPD(uint32_t address, uint32_t vtxCount, uint32_t dstIndex) {
        ...
        setVertexCommon<true, sizeof(VertexPD)>(address, rdramAddress, dstIndex, dstIndex + vtxCount);
    }
    ...
    void RSP::setVertexEXV1(uint32_t address, uint32_t vtxCount, uint32_t dstIndex) {
        ...
        setVertexCommon<false, sizeof(VertexEXV1)>(address, rdramAddress, dstIndex, dstIndex + vtxCount);
    }
    ...
    void RSP::addCurrentProjection(Projection::Type type) {
        ...
        fbPair.projections[projectionIndex].wr64MenuModelPreview = wr64MenuModelPreview(
            projectionMatrixSegmentedAddressStack[projectionMatrixStackSize - 1],
            wr64ViewportAddressStack[viewportStackSize - 1]);
        fbPair.projections[projectionIndex].wr64MenuWaterPreview = wr64RTMenuWaterViewport(
            wr64ViewportAddressStack[viewportStackSize - 1]);
    }
    ...
    void RSP::setVertexCommon(uint32_t segmentedAddress, uint32_t rdramAddress, uint32_t dstIndex, uint32_t dstMax) {
        const int workloadCursor = state->ext.workloadQueue->writeCursor;
        Workload &workload = state->ext.workloadQueue->workloads[workloadCursor];
        ...
        if (extended.modelMatrixIdStackChanged) {
            ...
            {
                TransformGroup &wr64Group = workload.drawData.transformGroups.back();
                const bool wr64SkyExact = (wr64Group.matrixId == 0x57524300U);
                if (wr64SkyTexcoordAutoEnabled() && wr64SkyExact) {
                    wr64Group.texcoordInterpolation = G_EX_COMPONENT_AUTO;
                }
                wr64SkyNoteMatrixId(wr64Group.matrixId, wr64SkyTexcoordAutoEnabled() && wr64SkyExact);
            }
...
#       endif
        ...
        if (addWorldTransform) {
            ...
            uint32_t transformGroupIndex = extended.curModelMatrixIdGroupIndex;
            const uint32_t segmentedAddress = modelMatrixSegmentedAddressStack[modelMatrixStackSize - 1];

            const Wr64ArticulatedMatrixInfo articulated = decodeWr64ArticulatedMatrixAddress(segmentedAddress);
            const Wr64SelectorMatrixInfo selector = decodeWr64SelectorMatrixAddress(physicalAddress);
            if (articulated.valid) {
                const uint32_t matrixId = wr64ArticulatedMatrixId(articulated,
                    wr64ViewportAddressStack[viewportStackSize - 1]);
                transformGroupIndex = uint32_t(workload.drawData.transformGroups.size());
                workload.drawData.transformGroups.emplace_back(makeWr64FinalMatrixGroup(matrixId, true));
            }
            else if (selector.valid) {
                const uint32_t matrixId = 0x57100000 | (selector.watercraftIndex << 8) | selector.componentIndex;
                transformGroupIndex = uint32_t(workload.drawData.transformGroups.size());
                workload.drawData.transformGroups.emplace_back(makeWr64FinalMatrixGroup(matrixId, false));
            }
            else if (isWr64CourseSelectorMapMatrixAddress(physicalAddress)) {
                transformGroupIndex = uint32_t(workload.drawData.transformGroups.size());
                workload.drawData.transformGroups.emplace_back(makeWr64FinalMatrixGroup(0x57200000, false));
            }

            workload.physicalAddressTransformMap.emplace(physicalAddress, uint32_t(worldTransformGroups.size()));
            worldTransformGroups.emplace_back(transformGroupIndex);
            worldTransformSegmentedAddresses.emplace_back(segmentedAddress);
        ...
        auto &vertexSegmentedAddresses = workload.drawData.vertexSegmentedAddresses;
        auto &wr64WaterProducerKeys = workload.drawData.wr64WaterProducerKeys;
        ...
        for (uint32_t i = dstIndex; i < dstMax; i++) {
            ...
            const uint32_t vertexSegmentedAddress = segmentedAddress + ((i - dstIndex) * vertexSize);
            vertexSegmentedAddresses.emplace_back(vertexSegmentedAddress);
            constexpr uint32_t Wr64InvalidWaterProducerKey = UINT32_MAX;
            constexpr uint32_t Wr64WaterVertexBase0 = 0x03013D68U;
            constexpr uint32_t Wr64WaterVertexBase1 = 0x030170D8U;
            constexpr uint32_t Wr64WaterVertexEnd = 0x030185E8U;
            constexpr uint32_t Wr64WaterProducerBase = 0x0160A98U;
            uint32_t producerKey = Wr64InvalidWaterProducerKey;
            uint32_t producerSlot = 0;
            if ((vertexSegmentedAddress >= Wr64WaterVertexBase0) &&
                (vertexSegmentedAddress < Wr64WaterVertexBase1))
            {
                producerSlot = (vertexSegmentedAddress - Wr64WaterVertexBase0) / 16U;
            }
            else if ((vertexSegmentedAddress >= Wr64WaterVertexBase1) &&
                (vertexSegmentedAddress < Wr64WaterVertexEnd))
            {
                producerSlot = (vertexSegmentedAddress - Wr64WaterVertexBase1) / 16U;
            }
            else {
                producerSlot = UINT32_MAX;
            }

            if (producerSlot != UINT32_MAX) {
                const uint8_t *producerBytes =
                    state->fromRDRAM(Wr64WaterProducerBase + (producerSlot * 8U));
                producerKey =
                    uint32_t(producerBytes[0]) |
                    (uint32_t(producerBytes[1]) << 8U) |
                    (uint32_t(producerBytes[2]) << 16U) |
                    (uint32_t(producerBytes[3]) << 24U);
            }

            wr64WaterProducerKeys.emplace_back(producerKey);
    ...
    void RSP::modifyVertex(uint16_t dstIndex, uint16_t dstAttribute, uint32_t value) {
        ...
        if (used[dstIndex]) {
            ...
            vertexSegmentedAddresses.emplace_back(vertexSegmentedAddresses[globalIndex]);
            wr64WaterProducerKeys.emplace_back(wr64WaterProducerKeys[globalIndex]);
    ...
    void RSP::setViewport(uint32_t address, uint16_t ori, int16_t offx, int16_t offy) {
        wr64ViewportAddressStack[viewportStackSize - 1] = address;
        const uint32_t rdramAddress = fromSegmentedMasked(address);
    ...
    void RSP::pushViewport() {
        if (viewportStackSize < RSP_EXTENDED_STACK_SIZE) {
            ...
            wr64ViewportAddressStack[viewportStackSize] = wr64ViewportAddressStack[viewportStackSize - 1];
