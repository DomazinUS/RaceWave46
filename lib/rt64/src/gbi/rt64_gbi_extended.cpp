...
namespace RT64 {
    namespace GBI_EXTENDED {
        ...
        void noOpHook(State *state, DisplayList **dl) {
            if ((*dl)->w0 == Wr64SplitLapMarker &&
                ((*dl)->w1 & 0xFFFFFFFCU) == Wr64SplitVersion && ((*dl)->w1 & 3U) != 3U) {
                state->flush();
                auto &workload = state->ext.workloadQueue->workloads[state->ext.workloadQueue->writeCursor];
                wr64DecodeLapMarker((*dl)->w0, (*dl)->w1, workload.wr64VerticalSplit,
                    workload.gameCallCount, workload.wr64LapScope);
                return;
            }
            if ((*dl)->w0 == Wr64SplitPauseMarker &&
                ((*dl)->w1 & 0xFFFFFFFCU) == Wr64SplitVersion && ((*dl)->w1 & 3U) != 3U) {
                state->flush();
                auto& workload = state->ext.workloadQueue->workloads[state->ext.workloadQueue->writeCursor];
                const uint8_t owner = uint8_t((*dl)->w1 & 3U);
                if (owner) {
                    workload.wr64PauseDrawOwner = owner;
                    workload.wr64PauseFirstCall = workload.gameCallCount;
                    workload.wr64PauseLastCall = 0;
                } else {
                    workload.wr64PauseLastCall = workload.gameCallCount;
                }
                return;
            }
            if ((*dl)->w0 == Wr64SplitMarker || (*dl)->w0 == Wr64SeparateWindowsMarker) {
                auto& workload = state->ext.workloadQueue->workloads[state->ext.workloadQueue->writeCursor];
                if (wr64DecodeSplitMarker((*dl)->w0, (*dl)->w1,
                        workload.wr64VerticalSplit, workload.wr64PauseOwner, &workload.wr64SeparateWindows)) return;
            }
