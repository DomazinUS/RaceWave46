...
namespace recomp {
    namespace config {
        ...
        public:
            ...
            // Override this config's storage directory without changing its file name or
            // the global config path. An empty path restores the normal directory.
            // Call load_config afterwards to load the selected directory's values.
            void set_config_directory(std::filesystem::path directory);
        ...
        private:
            ...
            // Preserve settings owned by another edition or a newer schema when saving.
            nlohmann::json extra_storage_options = nlohmann::json::object();
