...
namespace recomp {
    ...
    // Some ports are built from a ROM that is itself a patch of the cartridge
    // image. Registering a patcher lets select_rom accept the cartridge image a
    // player actually owns: it is called with the byte-order-normalised data
    // when that data does not already hash as the expected ROM, and returns
    // true if it recognised the data and converted it in place. Whatever it
    // produces is then hashed exactly as an unmodified selection would be, so a
    // patcher cannot widen what the runtime is willing to accept.
    void register_rom_patcher(std::function<bool(std::vector<uint8_t>&)> patcher);
    bool is_rom_valid(std::u8string& game_id);
    // Recheck a launcher's cached image without patching, rewriting or deleting it.
    bool validate_stored_rom(const std::u8string& game_id);
    bool is_rom_loaded();
