...
void Config::set_config_directory(std::filesystem::path directory) {
    config_directory = std::move(directory);
}
...
nlohmann::json Config::get_storage_json() const {
    nlohmann::json json = extra_storage_options;
...
bool Config::save_config_json(nlohmann::json config_json) const {
    std::filesystem::path directory = config_directory.empty() ? get_path_to_config(is_mod_config) : config_directory;
    ...
    std::filesystem::path file_path = directory / config_file_name;
...
bool Config::load_config(std::function<bool(nlohmann::json &)> validate_callback) {
    ...
    std::filesystem::path file_path = directory / config_file_name;
    nlohmann::json config_json{};
    bool found_config = read_json_with_backups(file_path, config_json);

    if (found_config && validate_callback != nullptr && !validate_callback(config_json)) {
        return false;
    }

    nlohmann::json *json_config_root = found_config ? get_config_storage_root(&config_json) : nullptr;
    extra_storage_options = json_config_root != nullptr && json_config_root->is_object()
        ? *json_config_root : nlohmann::json::object();
    storage.value_map.clear();
    temp_storage.value_map.clear();
    modified_options.clear();
    ...
    for (const auto& option : schema.options) {
        // A newly selected profile must start from schema defaults, never from
        // values (including unapplied edits) belonging to the previous profile.
        auto value = found_config
            ? parse_config_option_json_value((*json_config_root)[option.id], option)
            : get_option_default_value(option.id);
        storage.value_map[option.id] = value;
        extra_storage_options.erase(option.id);

        if (requires_confirmation) {
            temp_storage.value_map[option.id] = value;
        }
    }
    // Callbacks can read other options, so publish the complete profile first.
    for (const auto& option : schema.options) {
        auto value = get_option_value(option.id);
        try_call_option_change_callback(option.id, value, value, OptionChangeContext::Load);
    }

    derive_all_config_option_dependencies();
    if (loaded_config) {
        // Existing controls must refresh when the same Config switches profiles.
        for (size_t option_index = 0; option_index < schema.options.size(); option_index++) {
