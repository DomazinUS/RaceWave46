...
static std::function<bool(std::vector<uint8_t>&)> rom_patcher;

void recomp::register_rom_patcher(std::function<bool(std::vector<uint8_t>&)> patcher) {
    rom_patcher = std::move(patcher);
}
...
static std::vector<uint8_t> read_file(const std::filesystem::path& path) {
    ...
    if (file.good()) {
        ...
        // This helper only reads cartridge images/caches. Reject failed seeks
        // and unreasonable sizes before converting tellg() to an allocation.
        if (size <= 0 || size > 64 * 1024 * 1024) return {};
        ret.resize(static_cast<size_t>(size));
        file.seekg(0, std::ios::beg);

        if (!file.read(reinterpret_cast<char*>(ret.data()), ret.size())) return {};
    }
...
bool write_file(const std::filesystem::path& path, const std::vector<uint8_t>& data) {
    ...
    {
        std::ofstream out_file{ temporary, std::ios::binary | std::ios::trunc };
        if (!out_file.write(reinterpret_cast<const char*>(data.data()), data.size())) return false;
        out_file.flush();
...
bool recomp::validate_stored_rom(const std::u8string& game_id) {
    const auto entry = game_roms.find(game_id);
    if (entry == game_roms.end()) return false;
    const bool valid = check_hash(read_file(config_path / entry->second.stored_filename()), entry->second.rom_hash);
    if (valid) valid_game_roms.insert(game_id);
    else valid_game_roms.erase(game_id);
    return valid;
}
...
recomp::RomValidationError recomp::select_rom(const std::filesystem::path& rom_path, std::u8string& game_id) {
    ...
    // A registered patcher gets one chance to turn the cartridge image the
    // player owns into the ROM this port was built from. It is offered the data
    // only when the data is not already correct, and its output goes through
    // the same hash check as anything else.
    if (rom_patcher && !check_hash(rom_data, game_entry.rom_hash)) {
        if (rom_patcher(rom_data)) {
            rom_data.resize((rom_data.size() + 3) & ~3);
        }
    }
...
void ultramodern::quit() {
    exited.store(true);
    // VI/graphics can be parked by the diagnostic pause. Wake them before
    // start() joins those threads; main's final teardown runs only afterward.
...
void recomp::start(const recomp::Configuration& cfg) {
    ...
    ultramodern::join_thread_servicing_window_messages(game_thread);
    ultramodern::join_event_threads();
    ultramodern::join_thread_cleaner_thread();
    ultramodern::join_saving_thread();

    // An active diagnostic export may still be reading the RDRAM image.
    // Finish it while that allocation exists, after its producers have stopped.
